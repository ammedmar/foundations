...
In this section we collect some well-known, though not always easy to find in the literature, results about transversality and fiber products.
...
These conditions will be automatic in our setting, so Wedhorn's proof applies.
...
\end{corollary}
\begin{proof}
By definition, the map $f \times_M g$ is either of the compositions $f \pi_V = g \pi_W$. So $D_{(x,y)} (f \times_M g) = D_yg \circ D_{(x,y)} \pi_W$. On the other hand, considering the fiber product $D_xf \times_{T_z M} D_y g$ of the maps $D_x f \colon T_xV \to T_zM$ and $D_y g \colon T_yW \to T_zM$, it is the composition of the projection $T_xV \times_{T_zM} T_yW \to  T_yW$ and $D_y g$. But this projection is precisely $D_{(x,y}) \pi_W$, so the maps agree. 
\end{proof}
...
\begin{proof}
	...
	The second claim is now a special case of \cref{L: normal pullback}.
\end{proof}
...
\begin{proof}
	For simplicity of notation, let $P = V \times _MW$, and let $\pi_V \colon P \to V$ and $\pi_W \colon P \to W$ be the maps induced by the projections from $V \times W$ to $V$ and $W$.
...
\begin{remark}\label{R: multiproducts}
	...
	\greg{This needs a citation}
	

	However, the above equivalence between $n$-ary transversality and ``iterated transversality'' cannot hold in general, because the $n$-ary fiber product cannot necessarily know about all points involved in the iterated fiber product.
