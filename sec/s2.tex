...
We put this first result here, as some of the fundamental ideas will be used in a number of places throughout the text.
...
This will occasionally be useful below.
...
\begin{lemma}\label{L: nearby proper homotopy}
	...
	Then there exists a function $\varepsilon \colon X \to (0,\infty)$ such that every map $g \colon X \to M$ satisfying $d(f(x),g(x)) < \varepsilon(x)$ for all $x$ is proper and homotopic to $f$ by a proper homotopy.
...
We now apply the above results to demonstrate that we can make maps transverse via homotopies. 
...
\begin{theorem}\label{T: basic trans}
	...
	Furthermore, if $f$ is proper, then we can find such an $h$ so that both $h$ and $h(-,1)$ are proper. 
\end{theorem}
\greg{Old version of this had a bit about $M$ having boundary and keeping the $f$ restricted to boundary fixed if it was already good. Got rid of that to add proper without trouble. Need to make sure I never really need the boundary thing. Changeover around 9-11-23.}
...
\begin{proof}
	...
	This can be done by the smooth approximation theorem; see \cite[Corollary 9.2.32]{MaDo92}.
	...
	First, we want $f(x)+ \eta(x)s$ to land in $N_\epsilon$ for all $x$, and for this it is sufficient to have $\eta(x) = \epsilon(f(x))$. 
	If $f$ is not proper or we do not need the last clause of the theorem to hold, this will be sufficient. 
	...
	Clearly, $|f(x) - (f(x)+ \eta(x)s)| = |\eta(x)s| < \eta(x)$, and by definition $\pi$ takes points of $N_\epsilon$ to the nearest point in $N$. So, assuming $f(x)+ \eta(x)s \in N_\epsilon$, then $|f(x)+ \eta(x)s - \pi(f(x)+ \eta(x)s)| < \eta(x)|$.
	So by the triangle inequality, $|f(x) - H(x,s)| < 2\eta(x)|$.
	Thus if we take $\eta(x) < \min(\epsilon(f(x)), \varepsilon(x)/2)$, we will have $H$ well defined and both $f(x) - H(x,s)| < \varepsilon(x)$ for any $x \in M$ and $s \i D$ by \cref{L: nearby proper homotopy}. 

	Now, we have $H(x,0) = \pi(f(x)) = f(x)$, and, since $\eta(x)>0$ for all $x$, the map $(x,s) \to f(x)+ \eta(f(x))s$ is a submersion onto its image in $\R^K$.
	...
	While $h(-,1) = H(-,s_0)$ has the desired properties by construction, including being proper and properly homotopic to $f$ if $f$ is proper. \qedhere
