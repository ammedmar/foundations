...
\begin{definition}\label{D: smooth}
	If $A \subset \R^n$ and $B \subset \R^m$ are any subsets, we say that $f \colon A \to B$ is \textbf{smooth}\index{smooth!maps of subsets of Euclidean space|textbf} if it extends to a smooth map from an open neighborhood of $A$ to $\R^m$.
...
With this definition, the notions of smooth charts and atlases for manifolds in the standard setting can be extended to define (smooth) manifolds with corners; see \cite[Section 2]{Joy12}: An $n$-dimensional chart $(U,\phi)$ of the space $W$ has domain $U$ an open subset of $\R^n_k = [0,\infty)^k \times \R^{n-k} \subset \R^n$,\index{$\R^n_k$} and $\phi$ is a homeomorphism from $U$ to $\phi(U) \subset W$.
...
Inductively, we let $\bd^k W$\index{$\bd^k$|textbf} denote $\bd (\bd^{k-1} W)$ with $\bd^0 W = W$, and we let $i_{\bd^k W}$ denote the composite of the boundary maps sending $\bd^k W$ to $W$. The map $i_{\bd^k W}$ takes $S^a(\bd^k W)$ onto $S^{a+k}(W)$, but not, in general, injectively.
...
\begin{theorem}[$\epsilon$-Neighborhood Theorem]\label{T: epsilon neighborhood}\index{$\epsilon$-neighborhood theorem}
...
\subsubsection{Fiber products with more than two inputs}\index{fiber products!more than two inputs}
...
\begin{proposition}\label{P: 3 out of 4 trans}\index{fiber products!more than two inputs!redundancy of transversality conditions}
