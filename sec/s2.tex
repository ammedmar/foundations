...
However, it is observed in \cite[Problem 6-5]{Lee13} that there exists a tubular
neighborhood $U$ of $Y$ in $\R^M$ with retraction $r \colon U \to Y$ such that for each $z \in U$, $r(z)$ is the unique point of $Y$ closest to $z$.
The construction of $\epsilon$-neighborhoods in \cite{GuPo74} essentially finds them within any tubular neighborhood using the same retraction map for both the tubular neighborhood and the $\epsilon$-neighborhood, so by using tubular neighborhoods as described above, we may assume the closest-point characterization holds for our $\epsilon$-neighborhoods even when $Y$ is not compact.\greg{Add a proof?}
\end{remark}
