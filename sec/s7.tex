...
In \cref{S: trans apps}, we utilize this machinery along with our cubulations to construct a Kroneker pairing and a Universal Coefficient Theorem when the cohomology is finitely generated.
...
In \cref{S: cubical cup and cap}, we relate geometric (co)homology cup and cap products to their singular (co)homology counterparts more concretely, using cubulations and the intersection maps introduced in \cref{S: intersection map}.
We conclude with some applications to Poincar\'e Duality and umkehr maps.
...
After introducing some more transversality results in \cref{S: homology products} in the context of considering (co)homology products, we will return to this theme in \cref{S: trans apps}, where we consider pullbacks of cochains, a Kronecker pairing for geometric chains and cochains, and a universal coefficient theorem.
In \cref{S: exterior chain products}, we define exterior products of chains and cochains.
...
\begin{definition}\label{D: cochain trans}\index{transversality!simple|textbf}\index{geometric cochain!simply transverse|textbf}
	...
	We call the data of such a pair $(V,W)$ a \textbf{simple transverse representation}\index{simple transverse representation} for the pair $(\uV,\uW)$.
...
\begin{definition}\label{D: cochain products}
	Let $M$ be a manifold without boundary.
	For $\uV, \uW \in C^*_{\Gamma}(M)$ simply transverse, we define the \textbf{cup product}\index{cup product|textbf}\index{geometric cochain!cup product} $\uV \uplus \uW \in C^*_\Gamma(M)$ to be the geometric cochain represented by the fiber product $V \times_M W$ for some simple transverse representation $(V,W)$ of $(\uV,\uW)$.

	Analogously, if $\uV \in C^*_{\Gamma}(M)$ and $\uW \in C_*^{\Gamma}(M)$ are simply transverse, we define the \textbf{cap product}\index{cap product|textbf}\index{geometric cochain!cap product}\index{geometric chain!cap product} $\uV \nplus \uW \in C_*^\Gamma(M)$ by $V \times_M W$ for some simple transverse representation $(V,W)$.

	If $M$ is oriented and $\uV,\uW \in C_*^\Gamma(M)$ are simply transverse, we define the \textbf{intersection product}\index{intersection product|textbf}\index{geometric chain!intersection product} $\uV \bullet \uW \in C_*^\Gamma(M)$ by $V \times_M W$ for some simple transverse representation $(V,W)$.
...
\begin{theorem}\label{T: cochain product}\index{cup product|textbf}\index{geometric cochain!cup product}\index{cap product|textbf}\index{geometric cochain!cap product}\index{geometric chain!cap product}\index{intersection product|textbf}\index{geometric chain!intersection product}\index{simple transverse representation}
...
\begin{definition}\label{D: multicup}
	...
	Then we say that $\uV$ and $\uW$ are \textbf{compound transverse}\index{transversality!compound} and define the cup product $\uV\uplus\uW$ as $$\uV\uplus\uW = \sum_{i,j} \underline{V_i}\uplus \underline{W_j},$$
...
\begin{proof}
	...
	For each $V_{ij}$, we have its essential decomposition\index{essential decomposition} $$V_{ij} = V_{ij,E} \sqcup V_{ij,TI} \sqcup V_{ij,NI},$$
	and by \cref{T: minimal rep}, the cochain $\underline{V_{ij}}$ is also represented by a precochain of the form $Z_{ij} \sqcup V_{ij,NI}$, where $Z_{ij}$ is the minimal essential precochain\index{minimial essential prechain or precochain|textbf} of $\underline{V_{ij}}$.
...
\begin{definition}\label{D: exterior chain}\index{exterior product}\index{geometric cochain!exterior product}\index{geometric chain!exterior product}
...
\subsection{Properties of the chain and cochain products}\label{S: (co)chain properties}\index{cup product!properties|(}\index{cap product!properties|(}\index{intersection product!properties|(}\index{exterior product!properties|(}
...
\subsubsection{Boundary formulas}\index{cup product!boundary formula}\index{cap product!boundary formula}\index{intersection product!boundary formula}\index{exterior product!boundary formulas}
...
\subsubsection{Commutativity formulas}\index{cup product!commutativity}\index{cap product!commutativity}\index{intersection product!commutativity}\index{exterior product!commutativity}
...
\subsubsection{Unital properties}\label{S: unital properties}\index{cup product!unitality}\index{cap product!unitality}\index{intersection product!unitality}\index{exterior product!unitality}
...
\subsubsection{Mixed properties}\label{S: mixed formulas}\index{cup product!mixed product properties}\index{cap product!mixed product properties}\index{intersection product!mixed product properties}\index{exterior product!mixed product properties}
...
\begin{center}\index{cup product!is diagonal pullback of exterior product}\index{exterior product!is cup product of pullback}
	\begin{tabular}{|l|c|l|}
		\hline
...
\subsubsection{Immersion formulas}\index{cup product!of immersions}\index{cap product!of immersions}\index{intersection product!of immersions}\index{exterior product!of immersions}
...
\textbf{Naturality.}\index{cup product!naturality}\index{cap product!naturality}\index{intersection product!naturality}\index{exterior product!naturality}
...
\medskip\noindent\textbf{Associativity.}\index{cup product!associativity}\index{cap product!associativity}\index{intersection product!associativity}\index{exterior product!associativity}
...
\index{cup product!properties|)}\index{cap product!properties|)}\index{intersection product!properties|)}\index{exterior product!properties|)}
...
\begin{theorem}\label{T: (co)homology products}\index{cup product|textbf}\index{geometric cohomology!cup product}\index{cap product|textbf}\index{geometric cohomology!cap product}\index{geometric cohomology!cap product}\index{intersection product|textbf}\index{geometric homology!intersection product}\index{simple transverse representation}
...
\begin{theorem}\label{T: coho product properties}\index{cup product!properties}\index{cap product!properties}\index{intersection product!properties}\index{exterior product!properties}
...
One particular consequence of \cref{T: coho product properties} is that, by the first ``mixed'' property in \cref{S: mixed formulas}, the cup product in cohomology is the composition of the exterior product and the pullback by the diagonal map, as for singular cohomology.
While we could have taken this as the definition of the cohomology cup product and thus avoided some effort below in showing that the cohomology cup product as coming from the cochain cup product is well defined, it would not have been clear that we can always find transverse representatives $\uV$ and $\uW$ for two cohomology classes and so represent their cohomology cup product by the cochain cup product $\uV \uplus \uW$.
Consequently, in what follows we show that the cohomology cup product is well defined ``the hard way,'' but obtain several valuable results in their own right along the way.

In particular, to prove \cref{T: (co)homology products}, we will use the following important theorem.

\begin{theorem}\label{T: transverse reps}\index{geometric cohomology!transverse representative}\index{geometric homology!transverse representative}
Let $r_W \colon W \to M$ be a map from a manifold with corners to a manifold without boundary, and let $\uV \in H_*^\Gamma(M)$ or $\uV \in H^*_\Gamma(M)$.
Then there is an $r_V \colon V \to M$ representing $\uV$ such that $V$ is transverse to $W$.

Furthermore, if $r_W$ is proper, $V$ is transverse to $W$, and there is a pre(co)chain $Z$ such that $\bd Z \sqcup -V \in Q(M)$, i.e.\ $\uV = 0$ as a homology or cohomology class, then such a $Z$ can be chosen so that it is transverse to $W$.
\end{theorem}
...
\begin{proof}[Proof of \cref{T: (co)homology products}]
	...
	So $V' \times_M W$ and $V'' \times_M W$ are (co)homologous, and similarly for different choices of $W$.
	\qedhere
	\begin{comment}
...
\begin{proposition}\label{P: perturb transverse to map}\index{transversality!Transversality Theorem!universal}
	Suppose $r_V \colon V \to M$ and $r_X \colon X \to M$ are maps from manifolds with corners to a manifold without boundary and that $r_V$ is proper.
	Then there is a proper homotopy $H \colon M \times I \to M$ such that $H(-,0) = \id$ and $H(-,1)r_V \colon V \to M$ is transverse to $r_X$.

	Furthermore, if $r_X$ is proper and given another proper map from a manifold with corners $r_W \colon W \to M$ that is transverse to $r_X$, we can choose the homotopy $H$ above so that also the resulting universal proper homotopy of $W$ given by $W \times I \xr{r_W \times \id_I} M \times I \xr{H} M$ is transverse to $r_X$.
\end{proposition}
...
The following lemma says, roughly speaking, that when we construct such an embedding $e$, if $r_X$ is proper then $e(X)$ does not run off to infinity in the $\R^n$ factors over compact subsets of $M$.
...
\begin{proof}[Proof of \cref{P: perturb transverse to map}]
	...
	\begin{enumerate}
		...
		\item if $r_X$ is proper, there is a ball neighborhood $D_r^N$ of $0$ in $D^N$ such that for all $s \in D_r^N$ the composition $W \times \R^n \xr{r_W \times \id_{\R^n}} M \times \R^n \xr{F(-,s) \times \id_{\R^n}}M \times \R^n$ is transverse to $e$.
	\end{enumerate}
	...
	We may assume $\epsilon$ is bounded by $1$ (see \cite[Exercise 2.3.1]{GuPo74}).
	...
	We further note that up to this point we have not needed to assume $r_X$ to be proper, and the construction to this point is sufficient to prove the first paragraph of \cref{P: perturb transverse to map}.
	...
	For $W$ not compact, the first two properties listed above for $F$ will continue to hold, but the third relied on compactness and so need not hold any longer in general.
	...
	By our construction, for all $k\geq 0$ we have $\Upsilon_k(-,s) \times \id_{\R^n} \colon \bd^k W \times \R^n \to M \times \R^n$ transverse to $ei_{\bd^j X}$ at each $(w,s,z) \in \bd^k W \times D^N \times \R^n$ such that $(w,s) \in (r_Wi_{\bd^{k}W} \times \id_{D^N})^{-1}(M\times_\phi D^N) = \{(w,s) \in \bd^k W \times D^N \mid |s|<\phi(r_Wi_{\bd^k W}(w))\}$.
...
\begin{proof}[Proof of \cref{T: transverse reps}]
	Given \cref{P: perturb transverse to map}, the proof is very analogous to that of \cref{T: transverse complex}.

	Let $r_W \colon W \to M$ be a map from a manifold with corners to a manifold without boundary, and let $\uV \in C_*^\Gamma(M)$ (or $C^*_\Gamma(M)$) be a cycle (or cocycle).
	...
	We co-orient $H$ and $H(-,1)$ as in \cref{D: homotopy co-orientation} based on the tautological co-orientation of $H(-,0)$.
	By \cref{D: universal homotopy,P: universal homotopy}, $H(-,1)r_V$ represents the same homology or cohomology class as $V$.

	Next, suppose $r_W$ is proper, $V$ is transverse to $W$, and there is a pre(co)chain $Z$ such that $\bd Z \sqcup -V \in Q(M)$.
	...
	Let $Z'$ denote the precochain $Z \xr{H(-,1)r_Z} M$, let $V'$ be the precochain $V \xr{H(-,1)r_V} M$, and let $Y$ be the precochain $V \times I \xr{H\circ (r_V \times \id_I)} M$, co-orienting $r_V \times \id_I$ using the naive pre-homotopy co-orientation convention, \cref{homotopy product co-orientation convention}.
	Let $A$ be the precochain $Z' \sqcup -Y$, which is transverse to $r_W$.
	We note that $\bd Z' \sqcup -V'$ is the image of $\bd Z \sqcup -V$ after composing with $H(-,1)$, and so it is in $Q(M)$ by \cref{L: Q preservation}.
	We also have $\bd Y = V' \sqcup -V \sqcup B$, where $B$ is (up to co-orientation) the precochain $\bd V \times I \xr{H\circ (r_{\bd V} \times \id_I)} M$ (cf. the proof of \cref{C: homotopy}).
	We now compute
	\begin{align*}
...
As in \cref{R: countable trans}, and for the same reasons, the proof of \cref{T: transverse reps} allows us to extend the first part of theorem, concerning finding a representative $V$ of $\uV$ that is transverse to $W$, to finding a $V$ that is transverse to each of a countable collection $W_i$.
\end{remark}

While \cref{P: perturb transverse to map} as stated was what we needed to prove \cref{T: transverse reps}, we take the opportunity while in the midst of the machinery to prove the following generalization, which will be useful for other applications, particularly in \cite{GBF47}.

\begin{proposition}\label{P: perturb transverse to map; pairs}\index{transversality!Transversality Theorem!for collection of pairs}
	Suppose $r_V \colon V \to M$ and $r_X \colon X \to M$ are proper maps from manifolds with corners to a manifold without boundary.
	Then there is a proper homotopy $H \colon M \times I \to M$ such that $H(-,0) = \id$ and $H(-,1)r_V \colon V \to M$ is transverse to $r_X$.

	Furthermore, given a finite collection of pairs of proper maps of manifolds with corners $r_{W_i} \colon W_i \to M$ and $r_{Y_i} \colon Y_i \to M$ such that $r_{W_i}$ is transverse to $r_{Y_i}$ for all $i$, we can choose the homotopy $H$ above so that for each $i$ the resulting universal proper homotopy $W_i \times I \xr{r_{W_i} \times \id_I} M \times I \xr{H} M$ is transverse to $r_{Y_i}$ .
\end{proposition}
\begin{proof}
	First consider a single pair $(W,Y)$.
	We see in the part of the proof of \cref{P: perturb transverse to map} involving $W$ that we do not really need the $X$ there to be the same as the $X$ in the first part of the proof.
	If we replace $X$ in this part of the argument with $Y$ and choose the embedding $e$ to be an embedding $e \colon X \sqcup Y \into M \times \R^n$ for appropriate $n$, and if we assume that $W$ is transverse to $Y$ instead of $X$, then in the compact $W$ case the arguments are exactly the same to conclude there is a radius $r$ such that for all $s \in D_r^N$ the composition $W \times \R^n \xr{r_W \times \id_{\R^n}} M \times \R^n \xr{F(-,s) \times \id_{\R^n}}M \times \R^n$ is transverse to the restriction of $e$ to $Y$.
	Constructing $H$ using $s_0 \in D_r^N$, we conclude just as above that $W \times I \xr{r_W \times \id_I} M \times I \xr{H} M$ is transverse to $r_Y$.
	Similarly, if $W$ is not compact, we come to the same conclusions by modifying the argument to choose $\phi$ based on $Y$ instead of $X$.

	Taking this further, next suppose we have any finite collection of pairs $(W_i, Y_i)$ with each $W_i$ compact.
	By starting with an embedding $e \colon X \sqcup (\bigsqcup_i Y_i) \into M \times \R^n$ for appropriate $n$, the preceding paragraph says that we can find $D_{r_i}^N$ such that for all $s \in D_{r_i}^N$ the composition $W_i \times \R^n \xr{r_{W_i} \times \id_{\R^n}} M \times \R^n \xr{F(-,s) \times \id_{\R^n}}M \times \R^n$ is transverse to the restriction of $e$ to $Y_i$.
	Then choosing $r < \min\{r_i\}$, we end up with an $H$ such that that $W_i \times I \xr{r_{W_i} \times \id_I} M \times I \xr{H} M$ is transverse to $r_{Y_i}$ for each $i$, in addition to the original result that $H(-,1)r_V$ is transverse to $r_X$.

	For $W_i$ that are not necessarily compact, we further modify the argument as in the proof of the proposition by using open covers with compact closures to find functions $\phi_i \colon M \to \R$ as in that proof using the pairs $(W_i, Y_i)$.
	If we then let $\phi \colon M \to \R$ be such that $0 < \phi(y) < \min\{\phi_i(y)\}$ for all $i$ and all $y \in M$, the rest of the argument applies to show that again $W_i \times I \xr{r_{W_i} \times \id_I} M \times I \xr{H} M$ is transverse to $r_{Y_i}$ for all $i$, in addition to the original result that $H(-,1)r_V$ is transverse to $r_X$.
\end{proof}


\subsection{Further applications of transversality}\label{S: trans apps}

In this section, we gather together several developments that are available now that we have proven in \cref{T: transverse reps} that arbitrary cochains can be made transverse.

\subsubsection{Transverse subcomplexes}\label{S: product pullbacks}
In \cref{S: cohomology pullback}, we showed that a continuous map $f \colon N \to M$ of manifolds without boundary yields a well-defined cohomology map $f^* \colon H^*_\Gamma(M) \to H^*_\Gamma(N)$.
In this section, we consider $f^*$ as a partially-defined map of cochain complexes $f^* \colon C^*_\Gamma(M) \to C^*_\Gamma(N)$.
In general, $f^*$ will not be fully defined as a chain map, as we cannot form pullbacks of cochains that are not transverse to $f$.
To solve this problem, we define a subcomplex of $C^*_\Gamma(M)$ that will be quasi-isomorphic to $C^*_\Gamma(M)$ if $f$ is proper.

\begin{definition}\label{D: transverse to map}\index{geometric cochain!transverse to a map}
	Let $f \colon N \to M$ be a smooth map from a manifold with corners to a manifold without boundary, and let $\uV \in C^*_\Gamma(M)$.
	We will say that $\uV$ is \textbf{transverse to $f$} if $\uV$ has a representative $r_V \colon V \to M$ such that $r_V$ is transverse to $f$.
	In this case we define the pullback $f^*(\uV)$ to be $\underline{f^*(V)} \in C^*_\Gamma(N)$.

	We will write the set of cochains transverse to $f$ as $C^*_{\Gamma \pf f}(M)$.\index{$C^*_{\Gamma \pf f}(M)$}
\end{definition}

We notice that the transversality situation here is simpler than the more general ones in \cref{S: chain products}, as $f$ is a fixed map.
We also remark that, given a cubulation $X$ of $M$, it determines a proper map from the disjoint union of all the $\dim(M)$-cubes of $X$ to $M$, which we can also write $F \colon X \to M$ with an abuse of notation.
In this case, $C^*_{\Gamma \pf F}(M) = C^*_{\Gamma \pf X}(M)$ as defined in \cref{D: trans cube}.

\begin{proposition}\label{P: trans to f}\index{geometric cochain!transverse to a map}\index{geometric cochain!transverse to a map!pullback of}
	Given a smooth map from a manifold with corners to a manifold without boundary $f \colon N \to M$, the set $C^*_{\Gamma \pf f}(M)$ is a subcomplex of $C^*_{\Gamma}(M)$.
	If $\bd N = \emptyset$, then $f^*:C^*_{\Gamma \pf f}(M) \to C^*_{\Gamma}(N)$ is a well-defined chain map.
\end{proposition}

\begin{proof}
	If $\uV,\uW \in C^*_\Gamma(M)$ are represented by $r_V \colon V \to M$ and $r_W \colon W \to M$ that are transverse to $f$, then $\uV+\uW$ can be represented by $V \sqcup W$, which will also be transverse to $f$.
	So $C^*_{\Gamma \pf f}(M)$ is closed under addition.
	If $r_V \colon V \to M$ is transverse to $f$ then so is $-r_V$, i.e.\ $r_V$ with the opposite co-orientation, so $C^*_{\Gamma \pf f}(M)$ is closed under negation.
	The empty map $\emptyset \to M$ is always transverse to $f$ (since there are no points at which to check the tangent space condition), and so $0 \in C^*_{\Gamma \pf f}(M)$.
	Finally, if $\uV$ is represented by $r_V \colon V \to M$ transverse to $f$, then by definition $\bd V \to M$ is transverse to $f$, so $\bd \uV \in C^*_{\Gamma \pf f}(M)$.
	Therefore, $C^*_{\Gamma \pf f}(M)$ is a subcomplex of $C^*_{\Gamma}(M)$.

	Now suppose $\bd N = \emptyset$.
	To show that $f^*$ is well defined on $C^*_{\Gamma \pf f}(M)$ we must show that it does not depend on the choice of representative $V$.
	Suppose $V$ and $V'$ both represent $\uV$ and are transverse to $f$.
	Then $V \sqcup -V'$ is transverse to $f$ and an element of $Q^*(M)$.
	So by \cref{L: pullback map Q}, $f^*(V \sqcup -V')$, which is by definition $(V \sqcup -V') \times_M N = (V \times_M N) \sqcup (-V' \times_M N)$ mapping to $N$, is an element of $Q^*(N)$.
	So $f^*(V)$ and $f^*(V')$ represent the same element of $C^*_{\Gamma}(N)$.
	Thus $f^*$ is well defined.

	To see that $f^*$ is a homomorphism, let $V,W \to M$ represent elements of $C^*_{\Gamma}(M)$ that are transverse to $f$.
	Then
	...
	Furthermore, as $\bd N = \emptyset$, $f^*$ is a chain map by \cref{leibniz}.
\end{proof}
...
\begin{remark}
	...
	For more general constructions that address this issue, see \cite{GBF47}.
\end{remark}

Generalizing \cref{T: transverse complex}, we have the following.

\begin{theorem}\label{T: transverse qi}\index{geometric cohomology!transverse to a map!is geometric cohomology}
	If $f \colon N \to M$ is a proper map from a manifold with corners to a manifold without boundary, then the inclusion $C^*_{\Gamma \pf f}(M) \into C^*_\Gamma(M)$ is a quasi-isomorphism.
\end{theorem}
\begin{proof}
	The proof is identical to that of \cref{T: transverse complex}, using \cref{P: perturb transverse to map} in place of \cref{P: ball stability}.
\end{proof}

The following corollary is immediate from \cref{{T: transverse qi}} and the construction in \cref{D: cohomology pullback and homology transfer}.
It says that, in the case at hand, the cohomology map of \cref{D: cohomology pullback and homology transfer} is induced by a chain map after restricting to a subcomplex of $C^*_\Gamma(M)$.

\begin{corollary}\label{C: transverse cohomology pullback}\index{geometric cohomology!transverse to a map!pullback of}
	Let $f \colon N \to M$ be a proper map between manifolds without boundary.
	The following diagram commutes, in which the top map is induced by the chain map $f^*$, the vertical map is induced by the inclusion $C^*_{\Gamma \pf f}(M) \into C^*_\Gamma(M)$, and the bottom map is that of \cref{D: cohomology pullback and homology transfer}:
	\begin{equation*}
	\begin{tikzcd}
		H^*_\Gamma(N) & H^*(C^*_{\Gamma \pf f}(M)) \arrow[l]  \arrow[d,"\cong"]\\
		& H^*_\Gamma(M). \arrow[lu]
	\end{tikzcd}
	\end{equation*}
	In particular, the image of the top map is independent of the choice of smooth $f$ within its (not necessarily proper) homotopy class.
\end{corollary}

\subsubsection{Kronecker pairing}\label{S: Kronecker}\index{Kronecker pairing|(}
...
We consider here the extent to which this pairing corresponds to a map $C^i_\Gamma(M) \to \Hom(C_i^\Gamma(M),\Z)$.
This situation is closely related to the preceding discussion of pullbacks.

\begin{definition}\label{D: transverse to cohain}\index{geometric chain!transverse to a cochain}\index{$C_i^{\Gamma \pf \uV}(M)$}
...
\begin{proposition}\label{P: chain kronecker}
...
\begin{comment}
\cref{P: chain kronecker} can be extended in various ways to include coefficients.
For example, given an abelian group $G$, then tensoring with $G$ gives a homomorphism $C_i^{\Gamma \pf \uV}(M;G) \to G$, or, given $g \in G$ we can take $\uW \in C_i^{\Gamma \pf \uV}(M)$ to $\aug(\uV \nplus \uW)\otimes g$.
If $R$ is a commutative ring, we can make both of these extensions.
\end{comment}
...
Descending to homology and cohomology, when applying the cap product to cohomology and homology classes of the same degree, we can compose with the augmentation map $\aug \colon H_0^\Gamma(M) \to \Z$ of \cref{D: aug} to obtain a bilinear Kronecker pairing\index{Kronecker pairing!of (co)homology}
...
\subsubsection{The Universal Coefficient Theorem for geometric cohomology}\label{S: UCT}
When $H^i_\Gamma(M)$ is finitely generated, the Kronecker map $\alpha$ of the preceding section fits into a short exact sequence, just as for singular cohomology.

\begin{theorem}\label{T: UCT}\index{Universal Coefficient Theorem}
	If $H^i_\Gamma(M)$ is finitely generated, there is a split short exact sequence
	\[
...
\begin{proof}[Proof of \cref{T: UCT}]
	...
	We know that $K_i(X)$ is a free abelian group, so the bottom map of the diagram is a split surjection by the algebraic Universal Coefficient Theorem, with kernel $\Ext(H_{i-1}(K_*(X)),\Z)$.
	The commutativity of the diagram thus implies that the top map of the diagram is a split surjection with isomorphic kernel.
...
\begin{theorem}\label{T: intersection is cup product}\index{geometric cohomology!cup product!is singular cup product}
...
Our proof is based on an axiomatic characterization of the cup product on manifolds due to Kreck and Singhof \cite[Proposition 12]{Krec10b}.\index{Kreck-Singhof Theorem}
...
\begin{proposition}[Kreck and Singhof, Proposition 12 of \cite{Krec10b}]\label{P: Kreck-Singhof pairing}\index{Kreck-Singhof Theorem}
...
\begin{lemma}\label{L: chain cross compare}\index{geometric homology!exterior product!is singular homology exterior product}
	Let $M$ and $N$ be manifolds without boundary.
...
\begin{theorem}[K\"unneth Theorem]\label{T: homology kunneth}\index{K\"unneth Theorem!geometric homology}\index{geometric homology!K\"unneth Theorem}
	Let $M$ and $N$ be manifolds without boundary.
...
\begin{proposition}\label{P: cross product is cross product}\index{geometric cohomology!exterior product!is singular cohomology exterior product}
...
\begin{theorem}[K\"unneth Theorem]\label{T: cohomology kunneth}\index{K\"unneth Theorem!geometric cohomology}\index{geometric cohomology!K\"unneth Theorem}
...
The composition left to right will be our \textbf{cubical cochain cross product},\index{exterior product!cubical cochain} which we denote simply $\times$, by the standard convention.
...
\begin{proposition}\label{P: cross product comparison}\index{geometric cohomology!exterior product!is singular cohomology exterior product}
...
\subsubsection{Cubical cup and cap products}\label{S: cubical products}\index{cup product!cubical cohomology}\index{cap product!cubical cohomology}\index{cap product!cubical homology}
...
For an explicit formula relating the cap products, cf.\ \cref{T: equivalent cap} and the proof of \cref{C: cap relation}.
...
For cup products, it will be useful for us also to have formulas in terms of cross products.
It follows directly from the above definition that the singular cubical cup product satisfies the familiar formula $$\alpha \smile \beta = \diag^*(\alpha \times \beta),$$
...
\begin{definition}\label{D: Delta}\index{Serre diagonal}
...
The following proposition demonstrates the compatibility between the cubical, cubical singular, and smooth cubical singular cup products as written in terms of these ``diagonals.''

\begin{proposition}\label{P: cubical and singular cups}\index{cup product!cubical cohomology}
...
\begin{theorem}\label{T: cup compatibility}\index{geometric cohomology!cup product!is cubical cohomology cup product}
	...
	If $H^*(M)$ is finitely generated, then the vertical maps are isomorphisms.
		...
		\end{tikzcd}
		\]
	In other words, if $H^*(M)$ is finitely generated, the geometric and cubical cup products are isomorphic.

	Furthermore, this compatibility is natural with respect to maps of spaces in the sense of \cref{T: intersection natural}.
\end{theorem}
...
Furthermore, if we have any map $f \colon N \to M \times M$ that is transverse to every element of $C^*_{\Gamma \pf P}(M\times M)$ and if $\bd N = \emptyset$ , then we have a pullback chain map $f^* \colon C^*_{\Gamma \pf P}(M\times M) \to C^*_\Gamma(N)$ given by $f^*(\uV) = \underline{V \times_{M \times M} N}$ as a pullback mapping to $N$, where $V$ is any representative of $\uV$; cf.\ \cref{S: cohomology pullback} and the statement and proof of \cref{P: trans to f}.
In particular, this is the case for $\diag \colon M \to M \times M$, in which case we have a well defined chain map $\diag^* \colon C^*_{\Gamma \pf P}(M\times M) \to C^*_\Gamma(M)$.
...
\begin{proposition}\label{P: diag/intersect}\index{Serre diagonal!versus geometric diagonal}
...
We obtain the proof by concatenating the diagrams of \cref{P: cross product comparison} (after taking cohomology) and \cref{C: diag commute} and considering the rectangle formed on the outside. Naturality follows from \cref{T: intersection natural} and naturality of geometric, cubical, and simplicial cohomology.
\end{proof}


\subsubsection{A cup product with coefficients}\index{cup product!with coefficients}

In \cref{C: cohomology with coefficients}, we observed that if $H_*(M)$ is finitely-generated in each degree and $G$ is an abelian group, then $H^*(C^*_\Gamma(M) \otimes G) \cong H^i(M;G)$.
In this section we wish to extend this compatibility to cup products. We will consider only the cases $R=\Z_n$ and $R=\Q$.



In the following discussion, for clarity we will use notation such as $\uV$ for cochains and $[\uV]$ for cohomology classes.

\begin{definition}
	For a commutative ring with unit $R$ and manifold $M$, let $C^*_\Gamma(M;R) = C^*_\Gamma(M) \otimes R$ and $H^*_\Gamma(M;R) = H^*(C^*_\Gamma(M;R))$.
\end{definition}

In this section we will also write $1_n$ for the image of $1 \in \Z$ in $\Z_n$ and $1_\Q$ for the image in $\Q$.

To define the cup product $H^*_\Gamma(M;R) \otimes H^*_\Gamma(M;R) \xr{\uplus} H^*_\Gamma(M;R)$ when $R=\Z_n$, we recall that $C^*_\Gamma(M)$ is flat by \cref{L: flat} and tensor it with the short exact sequence $$0 \to \Z \xr{\times n} \Z \to \Z_n \to 0.$$
We then see that every element in $C^*_\Gamma(M;\Z_n)$ can be written as $\uV \otimes 1_n$ for some $\uV \in C^*_\Gamma(M)$, and $\uV$ is unique modulo cochains of the form $n \cdot \underline{Y}$.
When useful, we can also write $\uV \in C^*_\Gamma(M) = C^*_\Gamma(M)\otimes \Z$ as $\uV \otimes 1$.
Now if $[\uV \otimes 1_n], [\uW \otimes 1_n] \in H^*_\Gamma(M;\Z_n)$ and our representing cochains $\uV$ and $\uW$ are simply transverse, we define $[\uV \otimes 1_n] \uplus [\uW \otimes 1_n]$ to be represented by $(\uV \uplus \uW) \otimes 1_n \in C^*_\Gamma(M;\Z_n)$.

When $R=\Q$, we observe that for every element of $C^*_\Gamma(M;\Q) = C^*_\Gamma(M) \otimes \Q$, there is a $d \in \Z_+$ such that the element can be written as $\uV \otimes \frac{1}{d}$.
Of course $d$ is not unique and we have, for example, $\uV \otimes \frac{1}{d} = n\uV \otimes \frac{1}{nd}$ for $n \in \Z_+$.
If $\left[\uV \otimes \frac{1}{d}
\right]$ and $\left[\uV \otimes \frac{1}{e}
\right]\in H^*(M;\Q)$ with $\uV$ and $\uW$ simply transverse, we take their cup product to be $\left[(\uV \uplus \uW) \otimes \frac{1}{de} \right]$.
We observe that, as $C^*_\Gamma(M)$ is flat and $\Z \xr{\frac{1}{d}} \Q$ is injective for any $d$, the element $\uV \otimes \frac{1}{d}$ is $0$ in $C^*_\Gamma(M;\Q)$ if and only if $\uV = 0$ in $C^*_\Gamma(M)$.

In either case, letting $r$ stand for $1_n$ or $\frac{1}{d}$ and $s$ stand for $1_n$ or $\frac{1}{e}$, we see that $[\uV \otimes r] \uplus [\uW \otimes s]$ is represented by $\diag^*(\uV \times \uM) \otimes rs$ for appropriate choices of representing cochains $\uV$ and $\uW$.

We must ensure that these definitions always apply and do not depend on the choices made.

\begin{theorem}
	Let $M$ be a manifold without boundary and $R = \Z_n$ or $\Q$.
	The cup product $H^*_{\Gamma}(M;R) \otimes H^*_{\Gamma}(M;R) \to H^*_{\Gamma}(M;R)$ is fully defined and well defined.
\end{theorem}

\begin{proof}
	Let us first show that we can find appropriate simply transverse $\uV$ and $\uW$.

	For $\R = \Z_n$, we first choose arbitrary precochains $V$ and $W$ whose corresponding cochains $\uV$ and $\uW$ map to classes representing $[\uV \otimes 1_n]$ and $[\uW \otimes 1_n]$.
	By \cref{P: perturb transverse to map}, there is a proper universal homotopy that takes $V$ to a precochain $V'$ that is transverse to $W$.
	Following the proof of \cref{T: transverse reps}, we appeal to \cref{P: universal homotopy}, which relies on \cref{C: homotopy}.
	The proof of \cref{C: homotopy} together with \cref{L: co-oriented homotopy} shows that if $H$ is our homotopy of $M$ then taking a cochain $\underline{Z}$ to the cochain represented by $H\circ (r_Z \times \id_I)$ is a chain homotopy on $C^*_\Gamma(M)$, say $\mc H$ from the identity to $H(-,1)$.
	Thus $\mc H \otimes \id_{\Z_n}$ is a chain homotopy on $C^*_\Gamma(M;\Z_n)$,
	and we have $$\bd (\mc H \otimes \id_{\Z_n})(\uV \otimes 1_n) = (\uV'\otimes 1_n) - (\uV\otimes 1_n) -
	(\mc H \otimes \id_{\Z_n})\bd (\uV \otimes 1_n).$$
	But the last summand vanishes as $\uV \otimes 1_n$ is a cocycle by assumption.
	So we can always find a $\uV' \otimes 1_n$ representing $[\uV \otimes 1_n] \in H^*_\Gamma(M;\Z_n)$ such that $\uV'$ is simply transverse to $\uW$, and so we can take $[\uV \otimes 1_n] \uplus [\uW \otimes 1_n] = [(\uV \uplus \uW)\otimes 1_n]$.

	Similarly, for $R= \Q$, we obtain a chain homotopy between $\uV \otimes \frac{1}{d}$ and $\uV' \otimes \frac{1}{d}$ with $\uV'$ simply transverse to $\uW$.

	It remains to show that our resulting cohomology classes are independent of the choices. It suffices to show this for the first input, as the argument for the second input is identical. Again, we begin with $R = \Z_n$.

	Suppose given $\uV$ and $\uV'$ simply transverse to $\uW$ and such that $[\uV \otimes 1_n] = [\uV' \otimes 1_n].$
	Thus there is a $\uZ$ such that $\bd (\uZ \otimes 1_n)= (\uV'-\uV)\otimes 1_n$.
	By \cref{P: perturb transverse to map}, there is a proper universal homotopy that takes $\uZ$ to a $\uZ'$ that is simply transverse to $\uW$ and such that the trace of the homotopy of $\uV'-\uV$ is simply transverse to $\uW$.
	In this case, taking $\uX = \uZ' + \mc H(\uV'-\uV)$, the boundary of $\uX \otimes 1_n$ is $(\uV'-\uV) \otimes 1_n$.
	So by \cref{leibniz},
	$$\bd ((\uX \uplus \uW)\otimes 1_n) = ((\bd \uX) \uplus \uW)\otimes 1_n +(-1)^{m-x}(\uX \uplus \bd \uW)\otimes 1_n.$$
	By assumption $\bd \uW \otimes 1_n = 0$, so $\bd \uW$ must be $n$ times some element of $C^*_\Gamma(M)$.
	But then $\uX \uplus \bd \uW$ will also be a multiple of $n$, so the second term vanishes, while the first is $((\uV'-\uV)\uplus \uW) \otimes 1_n$.
	Furthermore, employing \cref{S: chain products}, this is $(\uV' \uplus \uW)\otimes 1_n - (\uV \uplus \uW) \otimes 1_n$, and so the class $[(\uV \uplus \uW)\otimes 1_n]$ does not depend on the choice of cochain representative of $[\uV \otimes 1_n]$.

	For $R=\Q$, by finding common denominators, we can write our cohomologous options as $(\uV'-\uV)\otimes \frac{1}{d}$ and the cohomology as $\uZ \otimes \frac{1}{d}$.
	The remaining argument is essentially identical to conclude $\left[(\uV \uplus \uW)\otimes \frac{1}{de}\right] = \left[(\uV' \uplus \uW)\otimes \frac{1}{de}\right]$.
\end{proof}

\begin{theorem}
	Let $M$ be a manifold without boundary cubulated by the cubical complex $X$, and suppose $H_*(M)$ is finitely generated.
	Let $R = \Z_n$ or $\Q$.
	Then the following diagram commutes, and, if $H_*(M)$ is finitely generated in all dimensions, the vertical maps are isomorphisms.
	\[
	\begin{tikzcd}
	H^*_\Gamma(M;R) \otimes H^*_\Gamma(M;R) \arrow[r,"\times"]\arrow[d,"\mc I \otimes \id_R \otimes \mc I \otimes \id_R"]& H^*_\Gamma(M \times M;R) \arrow[d,"\mc I\otimes \id_R"]\arrow[r,"\diag^*"] & H^*_\Gamma(M;R) \arrow[d,"\mc I \otimes \id_R"]\\
	H^*(K^*(X)\otimes R) \otimes H^*(K^*(X)\otimes R) \arrow[d] \arrow[r,"\times"]& H^*(K^*(X \times X)\otimes R) \arrow[r,"\Delta^*"]\arrow[d]  & H^*(K^*(X)\otimes R)\arrow[d] \\
	H^*(X; R) \otimes H^*(X; R) \arrow[r,"\times"]& H^*(X \times X;R) \arrow[r,"\Delta^*"] & H^*(X;R).
	\end{tikzcd}
	\]
	In other words, if $H_*(M)$ is finitely generated in all degrees, the geometric and cubical cup products with $R$ coefficients are isomorphic.
\end{theorem}
\begin{proof}
	All of the regions of the diagrams of \cref{E: cube products,C: diag commute} commute or commute up to homotopy at the chain level, and so they continue to commute or commute up to homotopy after tensoring with $R$, using that the tensor product of a chain homotopy with $\id_R$ is a chain homotopy and that $R \otimes_\Z R = R$ for $R = \Z_n$ or $\Q$.
	The quasi-isomorphisms remain quasi-isomorphisms by the Universal Coefficient Theorem and Five Lemma, recalling that we treat $R$ here as a $\Z$-module.
	So, as in the proof of \cref{T: cup compatibility}, we can put these diagrams together to obtain the top two rows of desired diagram.

	Furthermore, $H^*(K^*(X)\otimes R) \cong H^*(K^*(X;R))=H^*(X;R)$ by \cref{C: cohomology with coefficients}, and similarly for $X \times X$, so the lower vertical maps are isomorphisms.
	Commutativity of the bottom rows can be checked directly.
\end{proof}


\begin{comment}
Our focus will be primarily on the cup product for cohomology rather than at the cochain level, leaving that for other work.
However, to define the cup product with coefficients, we first assume elements of $\V,\W \in H^*_{\Gamma}(M;R) = H^*(C^*_\Gamma(M) \otimes R)$ represented by cochains $\sum_i \underline{V_i} \otimes r_i$ and $\sum_j \underline{W_j} \otimes s_j$ such that each pair $\underline{V_i}, \underline{W_j})$ is simply transverse.
Then we let $$\V \uplus \W = \sum_{ij} \left(\underline{V_i} \uplus \underline{W_j}\right) \otimes r_is_j.$$
We must show that this is always possible and independent of choices.

\begin{theorem}
	The cup product $H^*_{\Gamma}(M;R) \otimes H^*_{\Gamma}(M;R) \to H^*_{\Gamma}(M;R)$ is well defined.
\end{theorem}
\begin{proof}
	We first choose any cochain representative $\sum_j \underline{W_j} \otimes s_j$ for $\W$ and precochains $W_j$ representing each $\underline{W_j}$.
	Let $\mf V = \bigsqcup_i V_i$ and $\mf W = \bigsqcup_j W_j$.
	By \cref{P: perturb transverse to map}, there is a proper universal homotopy from $\mf V$ to a precochain $\mf V' =\bigsqcup_i V_i'$ such that $\mf V'$ is transverse to $\mf W$.
	Following the proof of \cref{T: transverse reps}, we appeal to \cref{P: universal homotopy}, which relies on \cref{C: homotopy}.
	The proof of \cref{C: homotopy} together with \cref{L: co-oriented homotopy} shows that if $H$ is our homotopy then taking $\uV$ to the cochain represented by $H\circ (r_V \times \id_I)$ is a chain homotopy, say $\mc H$.
	So in our case $\mc H \otimes \id_R$ provides a chain homotopy between the identity and $H(-,1)\otimes \id_R$, so $\sum \underline{V_i'}\otimes r_i = \sum_i \underline{V_i} \otimes r_i$ in $H^*_\Gamma(M;R)$.
	This shows that given any two cohomology classes, they have representatives that satisfy the required transversality.

	Next we show that the cup product is independent of choice of transverse representatives.

	Suppose $\{V_i\}$ and $\{U_k\}$ are such that $\sum \underline{V_i}\otimes r_i$ and $\sum \underline{U_k} \otimes t_k$ are two homologous elements of $H^*_\Gamma(M;R)$ and the $\{V_i\}$ and $\{U_k\}$ are all transverse to all elements of $\{W_j\}$ with $\sum \underline{W_j} \otimes s_j$ another element of $H^*_\Gamma(M;R)$.
	Let the cohomology be realized by $\sum Z_\alpha \otimes a_\alpha$.
...
\subsubsection{A fully defined cup product via flows}\label{S: flows}
...
\begin{theorem}[Theorem 1 of \cite{FMS-flows}]\label{T: FMMS}\index{cup product!via flows}
...
\begin{theorem}\label{T: equivalent cap}\index{cap product!is the cubical cap product}
...
\begin{corollary}\label{C: cap relation}\index{cap product!is the cubical cap product}
...
\begin{proof}
	...
	To relate with singular cubical cochains, we have equivalent formulas between cubical and singular cubical cap products, replacing $\mc J$ and $\mc I$ with the inclusion quasi-isomorphism $K_*(X) \into NK_*(M)$ of \cref{P: singular smooth cubes,T: cubical homology iso} and its dual quasi-isomorphism $NK^*(M) \to K^*(X)$.
	The cubical/singular cubical version of the formula of \cref{T: equivalent cap} follows from the discussion in \cref{S: cubical products}, as noted following the proof of \cref{L: EZ for cubical}.
\end{proof}
...
\subsection{Poincar\'e duality}\label{S: PD}
...
\begin{corollary}[Poincar\'e duality]\label{C: PD}\index{Poincar\'e Duality!via cap product}
...
\subsection{Umkehr maps}\label{S: umkehr}\index{umkehr maps|(}
...
\begin{proposition}\index{Poincar\'e Duality!and umkehr maps}\index{umkehr maps!and Poincar\'e Duality}
