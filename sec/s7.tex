...
\end{itemize}
If we assume that $\uV$ and $\uW$ are simply transverse, then we need only representatives $V$ and $W$ such that $V$ and $W$ are transverse and $V \times_M W$ is transverse to $h$.
...
For simple transversality versions, we need only assume $V, W, Z$ such that $(V, W)$, $(W,Z)$, $(V \times_M W,Z)$, and $(V, W \times_M Z)$ are transverse pairs.
...
\begin{proof}[Proof of \cref{T: (co)homology products}]
	...
	For the other products, we will show that if we are given homology or cohomology classes (depending on the particular product), then they can be represented by chains or cochains $\uV$ and $\uW$ that are simply transverse and that the product does not depend on such a choice.
	...
	Given (co)homology classes represented by $r_V \colon V \to M$ and $r_W \colon W \to M$, we will find a (proper) universal homotopy\footnote{Recall from \cref{S: covariant functoriality} that a universal homotopy will be one of the form $V \times I \xr{r_V \times \id} M \times I \xr{h} M$ and that such homotopies generate (co)homologies.} of $r_V$ to a map $r_V' \colon V \to M$ that is transverse to $W$.
	...
	To show that this gives a well-defined (co)homology class, we can suppose that $r_V'' \colon V \to M$ is another map transverse to $r_W$ representing the same (co)homology class as $r_V \colon V \to M$ with $r_Z:Z \to M$ providing the (co)homology.
	For notational purposes, let us write $r_V' \colon V' \to M$ and $r_V'' \colon V'' \to M$ with $V = V' = V''$.
	Then $\bd Z \sqcup V' \sqcup -V'' \in Q(M)$.
	...
	Now we can consider $\mc V \times_M W$, and we have $$\bd (\mc V \times_M W) = \pm \left( (\bd \mc V) \times_M W \right) \sqcup \pm \left( \mc V \times_M \bd W \right)$$ via the appropriate boundary formulas, with the precise signs depending on which kind of product we are considering (see \cref{P: oriented fiber boundary,leibniz,P: Leibniz cap}).
	As $W$ represents a (co)cycle, $\bd W \in Q(M)$ by \cref{R: cycles and boundaries}, so $\mc V \times_M \bd W \in Q(M)$ by \cref{L: pullback with Q}.
	Meanwhile, we have
	$$((\bd \mc V) \times_M W) \sqcup (V' \times_M W) \sqcup -(V'' \times_M W) = \left((\bd \mc V) \sqcup V' \sqcup -V''\right) \times_M W,$$
	which is again in $Q(M)$ by \cref{L: pullback with Q}.
	Altogether then,
	\begin{multline*}
	\bd (\mc V \times_M W)  \sqcup (V' \times_M W) \sqcup -(V'' \times_M W)\\
	  = \pm \left( (\bd \mc V) \times_M W \right) \sqcup \pm \left( \mc V \times_M \bd W \right) \sqcup (V' \times_M W) \sqcup -(V'' \times_M W)\\
	 = \left[\pm \mc V \times_M \bd W\right] \sqcup \left[ \pm \left((\bd \mc V) \times_M W \right) \sqcup (V' \times_M W) \sqcup -(V'' \times_M W)\right].
	\end{multline*}
	If we now replace $\mc V \times_M W$ in this last formula with $\pm \mc V \times_M W$ with the sign chosen so that we obtain $+(\bd \mc V) \times_M W$ in the bottom line, then the bottom line will be an element of $Q(M)$.
...
Then there is an $r_V \colon V \to M$ representing $\uV$ such that $V$ is simply transverse to $W$.

Furthermore, if $V$ is simply transverse $W$ and $\uV$ bounds (or cobounds), i.e.\ there is a pre(co)chain $Z$ such that $\bd Z \sqcup -V \in Q(M)$, then such a $Z$ can be chosen so that it is simply transverse to $W$.
\end{theorem}
...
To complete the proof above, we require \cref{P: perturb transverse to map}, below.
...
We change notation slightly from that of \cref{T: (co)homology products} to make it more consistent with \cref{P: ball stability}, which we hope will ease comparison of the two results for the reader.
...
\begin{proof}[Proof of \cref{P: perturb transverse to map}]
	...
	Then let $H(-,t) = F(-,ts_0)$, i.e.\ $H(x,t) = F(x,ts_0)$.
	...
	We define $f: M \times D^N \to M_\epsilon$ by $f(y, s) = y + \epsilon(y) s$, which is a submersion onto its image.
	...
	We also observe that the homotopy $H$ obtained from this $F$ by $H(x,t) = F(x,ts_0)$ is proper, as any sequence of $M \times I$ that ``escapes to infinity'' must similarly have image that escapes to infinity under $H$, using the boundedness of $\epsilon$.
	...
	Now, suppose\footnote{In this argument we will use the symbol $w$ to refer to a point in $W$, not the dimension of $W$.} $(w,z) \in W \times \bar B^n_0$, and let us fix $X_j$ as above.
	...
	Let $\hat f: M \times D^N \to M_\epsilon \subset \R^N$ be given by $\hat f(y, s) = y +\phi(y) \epsilon(y) s$; as $\phi(y)\epsilon(y)>0$, this is again a submersion onto its image at all points.
	...
	Letting $s_0$ be any such point we define $\hat H \colon M \times I \to M$ to be $\hat H(y,t) = \hat F(x,ts_0)$, and we claim that this $\hat H$ satisfies the conditions required by the proposition.

	The map $\hat H$ is proper, again because any sequence of $M \times I$ that ``escapes to infinity'' must similarly have image that escapes to infinity under $H$.
	...
	$$\hat h_0(w,t) = \pi_\epsilon(r_W(w)+\phi(r_W(w))\epsilon(r_W(w))ts_0).$$
