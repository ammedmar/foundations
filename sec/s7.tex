...
\begin{theorem}\index{cup product!properties}\index{cap product!properties}\index{intersection product!properties}\index{exterior product!properties}
...
In this section we let $G$ be a commutative ring with unit $R$ and show that \cref{T: cup compatibility} generalized to this case.
In other words, the geometric cup product with coefficient ring $R$ is isomorphic to the singular cup product with coefficient ring $R$.

Let us first define our cup product with coefficients, starting with a notational convention that is familiar for chains, though a bit unusual for cochains.
However, recall that geometric cochains are not defined as elements of a $\Hom$ group and that we introduce coefficients by taking tensor products.

\begin{notation}
	Let $M$ be a manifold and $R$ a commutative ring with unit.
	We may write elements $\uV \otimes r \in C^*_\Gamma(M) \otimes R$ as $r \uV$.
\end{notation}

\begin{definition} \label{D: cup with coeffs}
	Let $M$ be a manifold without boundary and $R$ a commutative ring with unit.
	Suppose we have two elements $r \uV, s \uW \in C^*_{\Gamma}(M) \otimes R$ and that $\uV$ and $\uW$ are simply transverse.
	We define the \textbf{cup product with coefficients}\index{cup product!with coefficients}\index{geometric cochain!cup product!with coefficients} $(r\uV) \uplus (s\uW) \in C^*_\Gamma(M)\otimes R$ to be $rs (\uV \ucup \uW) \in C^*_\Gamma(M) \otimes R$.
\end{definition}

\begin{lemma}
	The cup product in \cref{D: cup with coeffs} is well defined.
\end{lemma}
\begin{proof}
	We must show that the output is independent of choices, in this case
