...
\begin{theorem}
	Let $M$ be a manifold without boundary cubulated by the cubical complex $X$, and suppose $H^*(M)$ is finitely generated.
	Let $R = \Z_n$ or $\Q$.
	Then the following diagram commutes, and all the vertical maps are isomorphisms.
	\[
	\begin{tikzcd}
	...
	H^*(K^*(X)\otimes R) \otimes H^*(K^*(X)\otimes R) \arrow[d,"\cong"] \arrow[r,"\times"]& H^*(K^*(X \times X)\otimes R) \arrow[r,"\Delta^*"]\arrow[d,"\cong"]  & H^*(K^*(X)\otimes R)\arrow[d,"\cong"] \\
	H^*(X; R) \otimes H^*(X; R) \arrow[r,"\times"]& H^*(X \times X;R) \arrow[r,"\Delta^*"] & H^*(X;R).
	\end{tikzcd}
	\]
	In other words, the geometric and cubical cup products with $R$ coefficients are isomorphic.
\end{theorem}
\begin{proof}
	...
	Furthermore, $H^*(K^*(X)\otimes R) \cong H^*(K^*(X;R))=H^*(X;R)$ by \cref{C: cohomology with coefficients}, and similarly for $X \times X$, and these constructions can be done naturally by the techniques of \cite[Section 56]{Mun84}.
	This provides the commutativity of the bottom two rows.
\end{proof}
