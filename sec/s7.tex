...
To prove \cref{T: (co)homology products}, we use the following important theorem.
...
Then there is an $r_V \colon V \to M$ representing the same homology (or cohomology) class as $\uV$ such that $V$ is simply transverse to $W$.
...
\begin{proof}[Proof of \cref{T: (co)homology products}]
	...
	Given (co)homology classes represented by $r_V \colon V \to M$ and $r_W \colon W \to M$, we can find by \cref{T: transverse reps} a map $r_V' \colon V \to M$ that represents the same (co)homology class as $V$ and is simply transverse to $W$.
	...
	Then $\bd Z \sqcup V' \sqcup -V'' \in Q(M)$, so $V' \sqcup -V''$ bounds.
	We must show that $V' \times_M W$ and $V'' \times_M W$ are (co)homologous.

	Now by \cref{T: transverse reps}, we can replace assume that $Z$ is also simply transverse to $W$.
	\begin{comment}
	...
	\end{comment}
	So we can consider $Z \times_M W$, and we have $$\bd (Z \times_M W) = \pm \left( (\bd Z) \times_M W \right) \sqcup \pm \left( Z \times_M \bd W \right)$$ via the appropriate boundary formulas, with the precise signs depending on which kind of product we are considering (see \cref{P: oriented fiber boundary,leibniz,P: Leibniz cap}).
	...
	\end{multline*}
	If we now replace $Z \times_M W$ in this last formula with $\pm Z \times_M W$ with the sign chosen so that we obtain $+(\bd Z) \times_M W$ in the bottom line, then the bottom line will be an element of $Q(M)$.
...
\begin{proof}[Proof of \cref{T: transverse reps}]
	...
	Let $r_W \colon W \to M$ be a proper map from a manifold with corners to a manifold without boundary, and let $\uV \in C_*^\Gamma(M)$ (or $C^*_\Gamma(M)$) be a cycle (or cocycle).
	Let $r_V \colon C \to M$ be any representative for $\uV$.
	...
	Next, suppose $V$ is simply transverse to $W$ and there is a pre(co)chain $Z$ such that $\bd Z \sqcup -V \in Q(M)$.
	...
	Let $Z'$ be the precochain $Z \xr{H(-,1)r_Z} M$, let $V'$ be the precochain $V \xr{H(-,1)r_V} M$, and let $Y$ be the precochain $V \times I \xr{H\circ (r_V \times \id_I)} M$.
	Let $A$ be the precochain $Z' \sqcup -Y$, which is transverse to $r_W$.
	We note that $\bd Z' \sqcup -V'$ is the image of $\bd Z \sqcup -V$ after composing with $H(-,1)$, and so it is in $Q^*(M)$ by \cref{L: Q preservation}, using in the cochain case that we can co-orient $H$ and $H(-,1)$ as in \cref{S: co-oriented homotopy}.
	We compute
	\begin{align*}
		\bd A \sqcup -V &= \bd Z' \sqcup -\bd Y \sqcup -V\\
		&= \bd Z' \sqcup -(V' \sqcup -V) \sqcup -V\\
		&= \bd Z' \sqcup -V' \sqcup V \sqcup -V.
	\end{align*}
	We have already noted $\bd Z' \sqcup -V' \in Q^*(M)$ and $V \sqcup -V$ is trivial.
	So $\bd A \sqcup -V \in Q^*(M)$ and $A$ is transverse to the cubulation.
\end{proof}
