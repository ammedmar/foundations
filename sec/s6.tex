...
\subsection{Cubical complexes, cubulations, and cubical cochains}\label{S: cubes}

In this section, we develop notions and conventions concerning cubical structures and cubical cohomology.

\subsubsection{Cubical complexes}
We begin by recalling some notation from \cite{FMS-flows}.
In the context of cubical complexes we write the unit interval as $\interval = [0,1]$ and define the \textbf{standard $n$-cube}\index{cube} to be
\begin{equation*}
	\interval^n = \big\{ (x_1, \dots, x_n) \in \R^n\ |\ 0 \leq x_i \leq 1 \big\}.
\end{equation*}
Denote the set $\{1, \dots, n\}$ by $\overline{n}$.

Given a partition $F = (F_0, F_{01}, F_1)$\index{cube!partition of coordinates} of $\overline n$, it determines a \textbf{face}\index{cube!face|textbf} of $\interval^n$ given by
\begin{equation*}
...
We refer to coordinates $x_i$ with $i \in F_{01}$ as \textbf{free}\index{cube!free coordinates|textbf} and to the others as \textbf{bound}\index{cube!bound coordinates|textbf}.
...
Given any face $F$, its \textbf{initial vertex|textbf}\index{cube!initial vertex} is obtained by setting all free coordinates to $0$, and its \textbf{terminal vertex|textbf}\index{cube!terminal vertex} is obtained by setting all free coordinates to $1$.
...
\end{align*}
Any composition of these is referred to as a \textbf{face inclusion map}.\index{cube!face inclusion}
...
Analogous to the face and degeneracy identities for simplicial sets, these operators satisfy the following relations \cite[Section 4]{GrMa03} (it is also easy and illuminating to work these out on one's own):\index{cube!face/degeneracy identities}
...
An \textbf{interval subposet}\index{cube!interval poset|textbf} of $\mathcal P(\overline n)$ is one of the form $[v, w] = \{u \in \mathcal P(\overline n)\ |\ v \leq u \leq w\}$ for a pair of vertices $v \leq w$.
...
\begin{definition}\label{D:cubical}\index{cubical complex|textbf}
	...
	\begin{enumerate}
		...
		\end{equation*}
	\end{enumerate}
	We refer to an element $\sigma \in X$ as a \textbf{cube} or \textbf{face}\index{cubical complex!face} of $X$, refer to $\iota_\sigma \colon \sigma \to \mathcal P(\overline{n})$ as its \textbf{characteristic map},\index{cubical complex!characteristic map}
...
\noindent Given two cubical complexes $X$ and $Y$, their \textbf{product}\index{cubical complex!product of} is the cubical complex
\[
...
\subsubsection{Cubulations}\label{S: cubulation}

Let ${\tt Top}$ denote the category of topological spaces and continuous maps, and let ${\tt Cube}$\index{$Cu$@$\mathtt{Cube}$} be the subcategory whose objects are the $n$-cubes, identified with $\interval^n$, and whose morphisms are face inclusions.
...
We define the \textbf{geometric realization}\index{cubical complex!geometric realization|textbf} $|X|$ of $X$ as the colimit of this functor; in other words, we glue topological cubes together according to the combinatorial data of the cubical complex $X$ in the evident way.
A \textbf{cubical structure} or \textbf{cubulation}\index{cubulation|textbf} on a space $S$ is a homeomorphism $h \colon |X| \to S$ for some cubical complex $X$.
...
Smooth cubulations exist for any smooth manifold, as in the following construction of \cite{ShSh92}.\index{cubulation!smooth cubulations exist}
...
\subsubsection{Cubical chains and cochains}\label{S: cubical cochains}
...
The complex of \textbf{cubical chains}\index{cubical chain|textbf} of $X$, denoted $K_*(X)$,\index{$K_*(X)$} is defined to be the colimit of this composition.
...
We will write $H_*(X)$\index{$H_*(X)$} for $H_*(K_*(X))$, the cubical homology of $X$.\index{cubical homology}
...
For any face $F$ of $\interval^n$, the ordered subset $\beta_F = \{\e_i\ |\ i \in F_{01}\}$ defines the \textbf{canonical orientation} of $F$.\index{cubical complex!face!canonical orientation of}
...
The \textbf{cubical cochain complex}\index{cubical cochain|textbf} of $X$ (with $\Z$ coefficients) is the chain complex
\[
K^*(X) = \Hom_\Z(K_*(X), \Z).
\]\index{$K^*(X)$}
...
We will write $H^*(X)$\index{$H^*(X)$} for $H^*(K^*(X))$, the cubical cohomology of $X$.\index{cubical cohomology}
...
So we obtain a chain map, in fact a subcomplex inclusion, $\mc J \colon K_*(X) \into C^\Gamma_*(M)$.
...
\begin{theorem}\label{T: cubical homology iso}\index{cubical homology!is singular cubical homology}
...
\begin{corollary}\index{cubical cohomology!is cubical singular cohomology}
...
In this subsection we consider the cochains on $M$ represented by maps $W \to M$ that are transverse to a given cubulation of $M$.
While we focus primarily on cubulations, the same arguments apply for any smooth polyhedral subdivision of $M$, a fact that will be useful below.

\begin{definition}\label{D: trans cube}
	Let $M$ be equipped with a smooth cubulation $|X| \to M$.
	We say that $r_W \colon W \to M$ is \textbf{transverse}\index{cubulation!transversality to|textbf} to $X$ if $r_W \colon W \to M$ is transverse to each characteristic map of the cubulation.
	...
	If $r_W \colon W \to M$ is transverse to $X$ then the same is true for any $r_V \colon V \to M$ isomorphic to $r_W \colon W \to M$, and so we can define $PC^*_{\Gamma \pf X}(M)$\index{$PC^*_{\Gamma \pf X}(M)$} to be the subset of $PC^*_{\Gamma}(M)$ consisting of those precochains with reference maps transverse to $X$.
	...
	The \textbf{geometric cochains of $M$ transverse to $X$},\index{geometric cochain!transverse to cubulation|textbf} denoted $C_{\Gamma \pf X}^*(M)$,\index{$C_{\Gamma \pf X}^*(M)$} are the equivalence classes in $PC_{\Gamma \pf X}^*(M)$.
	...
	The \textbf{geometric cohomology transverse to $X$}\index{geometric cohomology!transverse to cubulation|textbf} is $H_{\Gamma \pf X}^*(M) \defeq H^*(C_{\Gamma \pf X}^*(M))$.\index{$H_{\Gamma \pf X}^*(M)$}
...
\begin{theorem}\label{T: transverse complex}\index{geometric cohomology!transverse to cubulation!is geometric cohomology}
...
\begin{proposition}\label{P: ball stability}\index{transversality!Transversality Theorem!universal with respect to cubulation}
	...
	Furthermore, given another proper map $r_W \colon W \to M$ from a manifold with corners that is transverse to the cubulation, we can choose the homotopy $H$ above so that also the resulting universal proper homotopy of $W$ given by $W \times I \xr{r_W \times \id_I} M \times I \xr{H} M$ is transverse to the cubulation.
\end{proposition}
...
\begin{proof}[Proof of \cref{T: transverse complex}]
	...
	By \cref{P: ball stability}, there is a proper homotopy $H \colon M \times I \to M$ such that $H(-,0) = \id_M$ and both $H(-,1)r_V$ and $H\circ (r_W \times \id_I)$ are transverse to the cubulation; co-orient $H$ and $H(-,1)$ as in \cref{D: homotopy co-orientation} based on the tautological co-orientation of $H(-,0)$.
	Let $V'$ be the precochain $V \xr{H(-,1)r_V} M$, let $W'$ be the precochain $W \xr{H(-,1)r_W} M$, and let $Y$ be the precochain $W \times I \xr{H\circ (r_W \times \id_I)} M$, co-orienting $r_W \times \id_I$ using the naive pre-homotopy co-orientation convention, \cref{homotopy product co-orientation convention}.
	Let $Z$ be the precochain $V' \sqcup -Y$, which is transverse to the cubulation.
	We note that $\bd V' \sqcup -W'$ is the image of $\bd V \sqcup -W$ after composing with $H(-,1)$, and so it is in $Q^*(M)$ by \cref{L: Q preservation}.
	Also by \cref{C: universal homotopy boundary co-orientation,L: co-oriented homotopy}, we have $\bd Y = W' \sqcup -W \sqcup B$, where $B$ is the map $\bd W \times I \xr{H\circ (r_{\bd W} \times \id_I)} M$ co-oriented as a homotopy from $-\bd W$ to $-\bd W'$.
	Note that $B$ is also transverse to the cubulation as part of the boundary of $Y$.
	We now compute
	\begin{align*}
...
\begin{lemma}\label{L: minimizer}\index{function subordinate to a cover with constants}
...
\begin{definition}\label{D: aug}
	We define the \textbf{augmentation map}\index{augmentation map}\index{$A$@$\aug$} $\aug \colon PC^\Gamma_0(M) \to \Z$ as follows: If $W \in PC^\Gamma_0(M)$, then $W$ is the disjoint union of a finite number of points, each with orientation denoted $1$ or $-1$.
...
\begin{definition}\label{D: intersection number}\index{intersection number|(}
	Suppose $M$ is an $m$-manifold without boundary and $W \in PC_\Gamma^i(M)$ and $N \in PC_{i}^\Gamma(M)$ are transverse.
	We define the \textbf{intersection number} $I_M(W,N)$ (or simply $I(W,N)$ if $M$ is clear from context) by $$I_M(W,N) = \aug(W \times_M N),$$ with $W \times_M N$ as defined in \cref{D: PC products}.
\end{definition}
...
\begin{definition}\label{D: intersection homomorphism}\index{intersection map|textbf}
...
\begin{proposition}\label{P: I is well defined}\index{intersection map!is well defined}
	The intersection map $\mc I$ is a well-defined chain map.
\end{proposition}

\begin{proof}
	If $W, W' \in PC^*_{\Gamma \pf X}(M)$ are two representatives of $\uW \in C^*_{\Gamma \pf X}(M)$ then $W \sqcup -W' \in Q^*(M)$ and it is transverse to $X$.
...
\begin{example}\label{E: uncountable}\index{geometric cochain!complex need not be free abelian}
...
\cref{T: intersection qi} is the formal statement of this theorem.
We will show below in \cref{T: intersection natural} that the intersection map in cohomology is natural with respect to maps in an appropriate sense.

\begin{theorem}\label{T: intersection qi}\index{intersection map!is a cohomology surjection}\index{intersection map!is a cohomology isomorphism when finitely generated}
	If $M$ is a manifold without boundary cubulated by $X$, then the intersection map $\mc I \colon H^i_{\Gamma \pf X}(M) \to H^i(X)$ is a surjection.
...
Before starting toward the proof, we provide an application to cohomology with coefficients and then show that we obtain similar isomorphisms with simplices instead of cubes.

\begin{corollary}\label{C: cohomology with coefficients}\index{geometric cohomology!with coefficients}
...
While we primarily utilize cubes for various technical reasons, it will be useful to also have a comparison of geometric and simplicial cohomology.
In particular, this is necessary for naturality results, as continuous maps of cubical complexes are not known to have cubical approximations.
If $C^*(T)$ denotes the simplicial cocochain complex of the simplicial complex $T$ that smoothly triangulates the manifold $M$, the intersection map $\mc I \colon C^*_{\Gamma(M) \pf T} \to C^*(T)$ is defined just as for cubical complexes.
To prove the following corollary regarding the induced map on cohomology, we do not need a completely separate proof; rather we can leverage \cref{T: intersection qi}.

\begin{corollary}\label{C: intersection simp qi}\index{intersection map!is a cohomology surjection}\index{intersection map!is a cohomology isomorphism when finitely generated}
	If $M$ is a manifold without boundary triangulated by $T$, then the intersection map $\mc I \colon H^i_{\Gamma \pf T}(M) \to H^i(T)$ is a surjection.
	If, in addition, $H^i(M)$ is finitely generated, then $\mc I$ is an isomorphism.
\end{corollary}
\begin{proof}
	As observed in \cref{S: cubulation}, a smooth triangulation can be subdivided into a smooth cubulation.
	Let $X$ be such a cubical subdivision of $T$ and consider the following diagram
	\begin{equation*}
		\begin{tikzcd}
		 	H^*_{\Gamma \pf X}(M)\arrow[d] \arrow[r,"\mc I"] & H^*(X)  \arrow[d]   \\
			H^*_{\Gamma \pf T}(M)\arrow[r,"\mc I"]&  H^*(T).
		\end{tikzcd}
	\end{equation*}
	The lefthand map is defined because any precochain transverse to $X$ is also transverse to $T$.
	It is also an isomorphism, employing that in the diagram $H^*_{\Gamma \pf X}(M) \to H^*_{\Gamma \pf T}(M) \to H^*_\Gamma(M)$, both the righthand map and the composition are isomorphisms by \cref{T: intersection qi}, which holds for any polyhedral subdivision of $M$.
	The righthand map is induced by the cellular subdivision chain map that takes an $i$-simplex to the sum of the $i$-cubes it contains (with appropriate signs), which induces an isomorphism on homology and so also on cohomology \cite[Theorem 1.49]{Fenn}.
	In particular, the underlying chain map right then down takes a geometric cocycle to a simplicial cocycle that acts on a simplex $\sigma$ by adding (with appropriate signs) the intersection numbers of the geometric cocycle with all the cubes contained in $\sigma$. But this is precisely the same as the map down then right after the easy verification that the signs work out.
 	So the diagram commutes, and the bottom map is a surjection or isomorphism when the top map is.
	The corollary now follows from \cref{T: intersection qi}.
\end{proof}
...
We can then write \index{$J$@$\jinterval^n$} $\jinterval^n = \left([0,1/2] \cup [1/2,1]\right)^n$, with the idea being that we consider $\interval^n$ as the union of $2^n$ \textit{subcubes} of side length $1/2$, each of which is the product within $\interval^n$ of $n$ factors, each factor equal to either $[0,1/2]$ or $[1/2,1]$.
We refer to $\jinterval^n$ with this structure as the \textbf{central subdivision}\index{cube!central subdivision|textbf} of $\interval^n$.
...
If no variable of such a face is bound to $0$ or $1$, we say that we have an \textbf{internal face of $\jinterval^n$},\index{$J$@$\jinterval^n$!internal face|textbf} otherwise we call it an \textbf{external} face.\index{$J$@$\jinterval^n$!external face|textbf}
External faces are all subsets of $\bd \interval^n$; internal faces are not subsets of $\bd \interval^n$.

To each face $F$ of $\interval^n$, we refer to the point $\hat F$ \index{$F$@$\hat F$} at which all its free variables are equal to $1/2$ as the \textbf{center}\index{cube!center|textbf} of $F$.
Each vertex is its own center.
To each face $F$ of $\interval^n$ we define its \textbf{dual face in $\interval^n$},\index{cube!dual face|textbf} or simply its \textbf{dual}, to be the face $F^\vee$ \index{$F^\vee$} of $\jinterval^n$ determined as follows:
\begin{itemize}
...
If $f$ is a face in $\jinterval^n$, we let $\bd_{\text{int}}f$ \index{$bd$@$\bd_{\text{int}}f$} denote the union of the internal $\dim(f)-1$ faces of $\bd f$ and $\bd_{\text{ext}}f$ \index{$bd$@$\bd_{\text{ext}}f$} denote the union of the $\dim(f)-1$ external faces of $f$.
...
We obtain the \textbf{centrally subdivided cubical complex $sd(X)$}\index{cubical complex!centrally subdivided} by replacing each cube $\interval^n$ with $\jinterval^n$.
...
\subsubsection{Pushing the dual cubulation}\index{cubulation!pushing the dual|(}
...
\end{proof}
\index{cubulation!pushing the dual|)}
...
\begin{theorem*}[\cref{T: intersection qi}]\index{intersection map!is a cohomology surjection}\index{intersection map!is a cohomology isomorphism when finitely generated}
...
\subsubsection{Naturality of the intersection map on cohomology}


Finally, we prove the following naturality statement.
We provide both cubical and simplicial statements, noting that one of our needs to consider triangulations at all is that a cubical approximation theorem is not known for cubical complexes!

\begin{theorem}\label{T: intersection natural}\index{intersection map!naturality}
	The cohomology intersection map $\mc I$ of \cref{T: intersection qi} is natural in the following sense.
	If $f \colon N \to M$ is a smooth map of manifolds without boundary and $f$ is homotopic to a cubical map $f_1 \colon Y \to X$ for respective smooth cubulations $Y$ and $X$ of $N$ and $M$, then the following diagram commutes:
	\begin{equation*}
		\begin{tikzcd}
		 	H^*_{\Gamma}(M) \arrow[d,"f^*"]	&\arrow[l,"\cong"'] H^*_{\Gamma \pf X}(M) \arrow[r,"\mc 	I"]\arrow[d,"f^*"]& H^*(X)\arrow[d,"f_1^*"]\\
			H^*_{\Gamma}(N) 	&\arrow[l,"\cong"'] H^*_{\Gamma \pf Y}(N) \arrow[r,"\mc I"]& H^*(Y).
		\end{tikzcd}
	\end{equation*}
	The equivalent result holds replacing cubulations with triangulations, in which case such a diagram always exists for any smooth $f \colon N \to M$ by taking $f_1$ to be a simplicial approximation.
\end{theorem}



\begin{proof}
	We first consider the cubical case.
	The map on the left is defined and depends only on the homotopy class of $f$ by \cref{P: cohomology pullback}.
	The leftwards maps are by inclusion and are isomorphisms by \cref{T: transverse complex}.
	For the middle vertical arrow and commutativity of the left square, it suffices to show that for every $\uV \in H^*_{\Gamma \pf X}(M)$ there is a precochain $V$ representing $\uV$ such that the $V$ is transverse to $X$ and the pullback $V \times_M N \to N$ is transverse to the cubulation $Y$.
	By \cref{L: transverse to pullback}, the latter condition is equivalent to each composition $fi_C \colon C \to M$ being transverse to $V$, where $i_C \colon C \to N$ is the inclusion of a cube of the cubulation of $N$.
	To achieve this, we utilize a generalization of \cref{P: ball stability,T: transverse complex} that we will prove below in \cref{S: homology products}.
	In particular, we can treat the smooth cubulation of a manifold as a smooth map from the disjoint union of the top dimensional cubes to the manifold.
	Let $\ms X$ be the union of cubes of $X$ and $\ms Y$ be the union of cubes of $Y$.
	Then the disjoint union of the cubulation $\ms X \to M$ and the composition $\ms Y \to N \xr{f} M$ can be considered a single map from a manifold with corners to $M$.
	By \cref{P: perturb transverse to map,T: transverse reps}, there is a precochain representing $\uV$ that is transverse to $\ms X$ and $\ms Y$.

	For the right square, if $V$ is a precochain representing $\uV \in H^*_{\Gamma \pf X}(M)$, then $f_1^* \mc I(\uV)$ is represented by the cubical cochain that takes a cube $C$ of $Y$ to $\aug(V \times_M f_1(C))$.
	This is well defined since $f_1(C)$ will be a cube of $X$, and so transverse to $V$ by assumption.
	On the other hand, $\mc I f^*(\uV)$ is represented by the cubical cochain that acts on $C$ by $\aug((V \times_M N) \times_N C)$, which can be made well-defined by choosing $V$ as in the preceding paragraph.
	By \cref{P: natural cap}, this last expression is the same as $\aug(V \times_M f(C))$.


	Now, by assumption $f$ and $f_1$ are homotopic, however $f_1$ may not be smooth.
	Nonetheless, let $F \colon N \times I \to M$ be the homotopy, which restricts to a continuous homotopy $C \times I \to M$ on each cube.
	We wish to modify $F$ to a homotopy $F'$ that is smooth and transverse to $V$ on each $C \times I$.
	Note that this is already the case on each $C \times 0$ and $C \times 1$ by assumption.
	We can proceed by induction on the dimensions of the cubes, modifying each $C \times I$ by a (continuous) homotopy rel $C \times \{0,1\}$ to obtain the desired $F'$.
	This can be done by the techniques of \cite{FMS-transversesingular}; that paper is written for simplices, rather than cubes, but the modification is direct.

	We thus obtain for any cube $C$ a fiber product $V \times_M (C \times I)$, with the mapping $C \times I$ to $M$ given by $F'$, and we define a cochain $A \in C^*(Y)$ by $A(C) = \aug(V \times_M (C \times I))$.
	We compute $$(dA)(C) = A(\bd C) = \aug(V \times_M ((\bd C) \times I)).$$
	We also observe that $\aug(V \times_M \bd (C \times I))$, the augmentation of the fiber product of $V$ with the full boundary of $C \times I$, must be $0$ because $V$ represents a cocycle and so $V \times_M (C\times I)$ consists of arcs with boundaries on the boundary of $C\times I$ (possibly plus elements of $Q^*(M)$).
	But $\bd (C \times I) = (\bd C) \times I \pm (C \times 1 - C \times 0)$,
	so $$(dA)(C) = \aug(V \times_M ((\bd C) \times I)) = \pm \aug(V \times_M (C \times 1) - V \times_M  (C \times 0))=\pm \aug(V\times_M f_1(C) - V \times_M f(C)).$$
	Hence $f_1^* \mc I(\uV)$ and $\mc I f^*(\uV)$ are cohomologous, as desired.

	The proof in the simplicial case is identical, noting that there is no need to triangulate the prisms $\sigma \times I$ in the argument.
	The final claim is due to the existence, for any continuous map of simplicial complexes, of a homotopic simplicial map, possibly after subdivision \cite[Section 16]{Mun84}.
\end{proof}
