...
\subsection{Cubulations}\label{S: cubulation}
...
We will show below in \cref{T: intersection natural} that the intersection map in cohomology is natural with respect to proper maps in an appropriate sense.
...
\begin{theorem}\label{T: intersection natural}\index{intersection map!naturality}
	...
	If $f \colon N \to M$ is a smooth proper map of manifolds without boundary and $f$ is properly homotopic to a cubical map $f_1 \colon Y \to X$ for respective smooth cubulations $Y$ and $X$ of $N$ and $M$, then the following diagram commutes:
	\begin{equation*}
		\begin{tikzcd}
...
\begin{proof}[Proof of \cref{T: intersection natural}]
	...
	In particular, we can treat the smooth cubulation of a manifold as a proper smooth map from the disjoint union of the top dimensional cubes to the manifold, essentially obtaining a precochain (we ignore the co-orientation).
	Let $\ms X$ be the union of cubes of $X$ and $\ms Y$ be the union of cubes of $Y$.
	Then the disjoint union of the cubulation $\ms X \to M$ and the composition $\ms Y \to N \xr{f} M$, is essentially a precochain in $M$.
	...
	The final claim is due to the existence, for any continuous map of simplicial complexes, of a homotopic simplicial map, possibly after subdivision.
	The construction of such a homotopy in \cite[Section 16]{Mun84} shows that the resulting simplicial approximation and the homotopy will be proper.
\end{proof}
