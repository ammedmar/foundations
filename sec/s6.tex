...
Analogously to simplicial complexes, cubical complexes possess algebraic cubical chain and cochain complexes and so cubical homology and cohomology that coincide with singular homology and cohomology\footnote{We will show below that cubical homology coincide with singular cubical homology, which coincide with simplicial singular homology by \cite{EM53}.
...
In analogy with the usual terminology in the simplicial setting, one could call these ``ordered cubical complexes," but we have no use for the unordered version.
...
\end{figure}
\greg{The example on the right of the figure \cref{F: cubical structure} is wrong - need to fix}
...
This is sufficient to determine a homomorphism $\Psi \colon K_X^*(M) \to C_\Gamma^*(M)$, thinking of elements of $K_X^*(M)$ as possibly infinite formal sums $\sum n_i F^*$ with the sum taken over all faces of a given dimension; compare \cite[Section 42]{Mun84}. Note that the image of such an element corresponds to a proper map from a manifold with corners to $M$, as the domain of such a precochain must be a countable collection of cubes, and their images in $M$ will be locally finite.
...
\begin{lemma}\label{L: push dual}
	...
	Then there is a smooth homotopy from the identity to a map $h \colon M \to M$ such that for each face $F$ of $X$ the following hold:
	\begin{enumerate}
...
\begin{proof}
	...
	We also ensure the global shift is small enough to provide the second condition wile also not disrupting the third.
	...
	The vector $v_f$ is also tangent to $f^\vee$ at $f \cap f^\vee = \hat f$, and for small $\delta>0$ the points $\hat f-\delta v_f$ lie in the interior of the face $f^\vee$.
	...
	Next, suppose we are given a small closed ball $D^f_r$ centered at $\hat f$ in $\R^m$ with $r<1/4$ so that $D^f_r$ does not intersect any faces of $\interval^m$ that do not intersect $f$ (and in particular $D^f_r$ does not intersect any other faces of the same dimension as $f$).
	Next choose numbers $0<r_4 < r_3 < r_2<r_1<r$, and let $D^f_{r_j}$ similarly be balls of radius $r_j$ centered at $\hat f$.
	...
	Let $\delta \ll \min\left\{\frac{r_4}{2}, \frac{r-r_1}{2}\right\}$, and let $\delta_1 < \delta$ be such that $\delta_1$ is smaller than any of the coordinates of $\delta v_f$.
	Next, let $z_f$ be a vector in $\R^m$ such that $|z_f-\delta v_f|\ll \delta_1$; by the condition on $\delta_1$, the signs of the coordinates of $z_f$ if $f_0$ and $f_1$ agree with those of $\delta v_f$, so $z_f$ continues to point outward from $\interval^m$ at all points of $f$.
	Finally, let $Z$ be the vector field on $\R^m$ given by $Z(x) = \eta(x)z_f$, and let $\Phi \colon \R^m \to \R^m$ be the map obtained by flowing by $Z$ from $t = 0$ to $t = 1$.
	Outside of $D^f_{r_1}$, the map $\Phi$ is the identity.
	We note $|z_f|\leq \delta+\delta_1 \leq 2\delta < r_4$, so the interior of $D_{r_4}^f$ (and hence all $D_{r_j}^f$) contains both $\hat f$ and $\hat f-z_f$.
	On $D_{r_2}^f$, the map $\Phi$ acts identically to the translation of the preceding paragraph.
	As $|z_f| < r-r_1$, and no point moves further than $|z_f$ under $\Phi$, we have $\Phi(D^f_r)\subset D^f_r$, so in particular the distance between the set $\Phi(f^\vee-(f^\vee \cap W^2_f))$ and the union of $\dim(f)$-faces of $\interval^m$ is positive.
	Also, as $z_f$ points outward from $\interval^m$ in the $f_0$ and $f_1$ directions, we have that $\Phi(\interval^m)$ contains $f$ with $\hat f$ in its interior.

	We now translate this procedure to the cubulation of $M$.
	Suppose $F$ is a face of the cubulation $X$, and choose an $m$-cube $B$ of the cubulation containing $F$ as a face.
	...
	Then $\phi$ takes $\hat f$ to the center $\hat F$ and $f^\vee_{\interval^m}$, the dual of $f$ in $\interval^m$, to one of the components of $F^\vee$.
	...
	As $\phi$ is an embedding at $\hat f$, by the Inverse Function Theorem we may choose $U$ such that $\phi_U$ is a diffeomorphism from $U$ onto its image, and by making $U$ smaller if necessary, we can also assume $U$ maps by $\phi_U$ to a Euclidean ball $D^F_r$ centered at $\hat f$ whose image in $M$ intersects only faces of $X$ that contain $F$.
	Identifying this ball in $\R^m$ with its image in $M$ via $\phi$, we can use this $r$ to choose quantities and perform a flow $\Phi$ as in the preceding paragraph to push $F^\vee_B$ into transverse position with $F$ (note: we keep $F$ fixed under the flow) and with intersection number $1$.
	We will henceforth abuse notation and use the same symbols for subsets of $\R^m$ and the corresponding subsets of $M$.
	Furthermore, this procedure pushes the other components $F^\vee_C$ for $C\neq B$ away from $F$ (slightly), and so $I(\Phi(F^\vee),F) = 1$ and also the distance between the set $\Phi(F^\vee-(F^\vee \cap D_{r^4}^F))$ and the union of $\dim(F)$-faces of $\interval^M$ is positive.
	As in the proof of the Transversality Theorem of \cite{GuPo74}, by rechoosing our $z_f$ if necessary (still with $|z_f - \delta v_f| < \delta_1$), we can ensure that the restriction of $\Phi$ to the intersection of any face of $sd(X)$ with $int(D_{r_2}^F)$ is transverse to the cubulation\footnotemark.

		\footnotetext{Let $e$ and $c$ be respectively the intersections of faces of $sd(X)$ and $X$ with $U$; via $\phi$ we identify these as submanifolds of $\R^m$.
		On the $D_{r_2}^F$ in $\R^m$, the flow acts by translation.
		...
		As a finite number of cubes of $X$ intersect $U$, almost all vectors in any neighborhood of $v_f$ will satisfy the claim.}

	Next, we can apply this procedure at all faces $F$ simultaneously by choosing a sufficiently small $D^F_r$ neighborhood of each $\hat F$ so that they are all disjoint; we let each $r$, $r_1$, $r_2$, $\delta$ and $\delta_1$ depend on $F$ though we do not include this in the notation.
	...
	As we are happy with the map $\Phi$ as constructed so far on the interior of $W_2$, we let $\rho \colon M \to [0,1)$ be a smooth function that is $0$ on $W_3$ and $>0$ on $M-W_3$.
	...
	In particular, each $H|_{F^\vee}(-,s_0)$ determines an element of $PC^*_{\Gamma \pf X}(M)$, co-orienting the map using that it is homotopic to the embedding of $F^\vee$ into $M$ (see \cref{S: co-oriented homotopy}) and that $F^\vee$ is normally co-oriented by the orientation of $F$.

	To complete the proof we must do one last thing: we must fine tune $\rho$ to ensure that in forming $H(-,s_0)$ to obtain the required transversality we have not pushed any $F^\vee$ so far as to create new intersections with faces of $X$ of complementary dimensions beyond the single intersection of $H(F^\vee,s_0)$ with $F$ inside $W_2$ that was obtained early using the flow $\Phi$.
	...
	So now if we replace $H$ with $H_2 \colon M \times S \to M$ defined by $H_2(x,s) = \pi(\Phi(x)+\rho_2(x)\epsilon s)$, then for any $s_0$ and for any $x \in (M-W_4) \cap F^\vee$, we have that $H_2(x,s_0)$ does not intersect any face of dimension $\dim(F)$ as desired.
...
At last, we have all the pieces needed to prove \cref{T: intersection qi}, which we restate here:
...
\begin{proof}
	In \cref{S: dualization of complexes} we constructed a chain map $\Psi \colon K_X^*(M) \to C_\Gamma^*(M)$ by taking $F^*$ to the geometric cochain represented by the inclusion of $F^\vee$ into $M$, but the image did not lie in $C^*_{\Gamma \pf X}(M)$.
	We now modify that construction to define $\psi:K_X^*(M) \to C_{\Gamma \pf X}^*(M)$ by taking $F^*$ to the element of $C^*_{\Gamma \pf X}(M)$ represented by the composition of the inclusion of $F^\vee$ into $M$ with the map $h$ constructed in \cref{L: push dual}.
	...
	Now, we know from \cref{T: geometric is singular} and the isomorphism between cubical and singular cubical cohomologies that these groups are both isomorphic to $H^*(M)$, and a surjective map of isomorphic finitely-generated abelian groups is an isomorphism\footnotemark.
\end{proof}

\footnotetext{Let $A$ be a finitely-generated abelian group, and let $f \colon A \to A$ be a surjection.
...
Now suppose $x \in A$ with $x \neq 0$.
As $f$ is surjective, $x = f^n(y)$ for some $y \in A$, and as $x \neq 0$, also $y \neq 0$.
Suppose $f(x)=0$.
Then $f(f^n(y)) = f^{n+1}(y)=0$.
But $\ker(f^n) = \ker(f^{n+1})$, so $f^n(y) = x = 0$, a contradiction.
So $f(x)=0$ only if $x=0$.
Thus $f$ is injective and surjective, i.e.\ an isomorphism.}
