...
\begin{example}\label{E: uncountable}
	...
	As $K^0(Z) = \Hom(K_0(Z),\Z) \cong \Hom\left(\displaystyle\oplus_{i \in \Z} \Z, \Z\right) \cong \prod_{i\in \Z} \Z$, this shows that the geometric cochain groups $C^*_\Gamma(M)$ need not be free abelian.
...
\begin{corollary}
	If $H_*(M)$ is finitely-generated in each dimension and $G$ is an abelian group, then $H^*(C^*_\Gamma(M) \otimes G) \cong H^i(M;G)$.
\end{corollary}
\begin{proof}
	...
	Now by Step 2 of the proof of \cite[Theorem 56.1]{Mun84}, $\Hom(F_*, \Z) \otimes G \cong \Hom(F_*, G)$, and this is quasi-isomorphic to $\Hom(K_*(X), G)$, again by \cite[Theorem 45.5]{Mun84}.
\end{proof}
