...
So we obtain a chain map, in fact a subcomplex inclusion, $\mc J \colon K^X_*(M) \into C^\Gamma_*(M)$ with $K^*_X(M)$ canonically isomorphic to $K_*(X)$.
...
There is also a map $\eta: K_*(X) = K^X_*(M) \to NK^{sm}_*(M)$ that takes each cube into its embedding into $M$.
Then $\mc J = \phi\eta \colon K^X_*(M) \to C^\Gamma_*(M)$.


\begin{theorem}\label{T: cubical homology iso}
	The map $\eta: K^X_*(M) \to NK^{sm}_*(M)$ induces an isomorphism of homology groups $H_*(K_*(X)) \to H_*(NK^{sm}_*(M))$, and the map $\mc J \colon K^X_*(M) \to C^\Gamma_*(M)$ induces an isomorphism of homology groups $H_*(K^X_*(M)) \to H_*^\Gamma(M)$.
\end{theorem}
...
\begin{proof}
	...
	\greg{Need to relate $\rho_2$ to distance in $M$ from $\Phi(x)$. Should also improve properness proof below to be }
