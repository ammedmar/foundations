...
\begin{theorem}\label{T: intersection natural}\index{intersection map!naturality}
	...
	If $f \colon N \to M$ is a smooth proper map of manifolds without boundary and $f$ is homotopic to a cubical map $f_1 \colon Y \to X$ for respective smooth cubulations $Y$ and $X$ of $N$ and $M$, then the following diagram commutes:
	\begin{equation*}
		\begin{tikzcd}
...
\begin{proof}
	...
	This can be done by the techniques of \cite{FMS-transverse singular}; that paper is written for simplices, rather than cubes, but the modification is direct.
