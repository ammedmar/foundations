...
Let ${\tt Top}$ denote the category of topological spaces and continuous maps, and let ${\tt Cube}$\index{$\mathtt{Cube}$} be the subcategory whose objects are the $n$-cubes, identified with $\interval^n$, and whose morphisms are face inclusions.
...
\begin{definition}\label{D: trans cube}
	...
	The \textbf{geometric cochains of $M$ transverse to $X$},\index{geometric cochains!transverse to cubulation|textbf} denoted $C_{\Gamma \pf X}^*(M)$,\index{$C_{\Gamma \pf X}^*(M)$} are the equivalence classes in $PC_{\Gamma \pf X}^*(M)$.
...
\begin{theorem}\label{T: transverse complex}\index{geometric cohomology!transverse to cubulation!is geometric cochomology}
...
\begin{definition}\label{D: aug}
	We define the \textbf{augmentation map}\index{augmentation map}\index{$\aug$} $\aug \colon PC^\Gamma_0(M) \to \Z$ as follows: If $W \in PC^\Gamma_0(M)$, then $W$ is the disjoint union of a finite number of points, each with orientation denoted $1$ or $-1$.
...
We can then write \index{$\jinterval^n$} $\jinterval^n = \left([0,1/2] \cup [1/2,1]\right)^n$, with the idea being that we consider $\interval^n$ as the union of $2^n$ \textit{subcubes} of side length $1/2$, each of which is the product within $\interval^n$ of $n$ factors, each factor equal to either $[0,1/2]$ or $[1/2,1]$.
...
If no variable of such a face is bound to $0$ or $1$, we say that we have an \textbf{internal face of $\jinterval^n$},\index{$\jinterval^n$!internal face|textbf} otherwise we call it an \textbf{external} face.\index{$\jinterval^n$!external face|textbf}
External faces are all subsets of $\bd \interval^n$; internal faces are not subsets of $\bd \interval^n$.

To each face $F$ of $\interval^n$, we refer to the point $\hat F$ \index{$\hat F$} at which all its free variables are equal to $1/2$ as the \textbf{center}\index{cube!center|textbf} of $F$.
...
If $f$ is a face in $\jinterval^n$, we let $\bd_{\text{int}}f$ \index{$\bd_{\text{int}}f$} denote the union of the internal $\dim(f)-1$ faces of $\bd f$ and $\bd_{\text{ext}}f$ \index{$\bd_{\text{ext}}f$} denote the union of the $\dim(f)-1$ external faces of $f$.
