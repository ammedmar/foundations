...
\cref{S: boundaries} explains boundaries of manifolds with corners, and \cref{S: first transversality} begins our treatment of transversality. Here again we rely on \cite{Joy12} for transversality of manifolds with corners. To avoid the most technical situations, we only consider transversality within manifolds without boundary.
We also review the classical transversality theorems that can be found, for example, in \cite{GuPo74} and
begin our development of transversality via \textbf{universal homotopies}; these are homotopies of maps $f \colon V \to M$ obtained by composing $f$ with a homotopy of the identity map of $M$.
Such homotopies are needed to make pairs of geometric chains and cochains transverse.
We start with basic versions of such results in \cref{T: basic trans,T: homotopy trans}.

In \cref{S: pullbacks} we review pullbacks and fiber products, including Joyce's result that the fiber product of transverse manifolds with corners (over a manifold without boundary) is again a manifold with corners \cite{Joy12}.
This is the key construction for cup and cap products of geometric cochains and chains.
We briefly consider notions of transversality for more than two objects in \cref{S:multitransversality}.
...
Our small rank condition for degeneracy replaces Lipyanskiy's notion of small image; we have found small rank technically easier to work with.
...
A satisfying consequence of these definitions is that geometric chains and cochains are tautologically identical on closed oriented manifolds, which immediately implies Poincar\'e Duality as stated in \cref{T: PD}.
In \cref{S: prechain products}, we develop proto versions of cup, cap, and intersection products for transverse prechains and precochains.
...
We place this material here to help the reader get a better sense for geometric chains and cochains, but the true applications are to transverse chain and cochain products in \cref{S: products}.
...
Creasing, also due to Lipyanskiy and considered in \cref{S: creasing}, is the technique by which we show that a geometric (co)chain represents the same (co)homology class as the sum of the two pieces determined by a splitting.
...
We first demonstrate the isomorphisms of (co)homology theories abstractly by applying a result of Kreck and Singhof \cite{Krec10b}, followed by demonstrating in \cref{S: homology direct} that the homology isomorphism can be obtained simply by considering smooth singular cubes (or singular simplices) as geometric prechains.
...
The main result of \cref{S: chain products} is that transversality of (co)chains is a well defined concept even though the same (co)chain can have multiple pre(co)chain representatives.
This is used to define cup, cap, and intersection products on transverse geometric (co)chains.
We here consider some conditions under which such pairings are bilinear; an alternative, more extensive treatment will be given in \cite{GBF47}.
We also introduce (co)chain-level exterior products.

In \cref{S: (co)chain properties}, we apply our previous results about pullbacks and fiber products to obtain all the expected properties of cup, cap, intersection, and exterior products, including boundary formulas (Leibniz rules), graded commutativity, unital properties, naturality formulas, associativity, and formulas involving multiple types of products simultaneously.
...
Continuing to apply our transversality results in \cref{S: trans apps}, we show in \cref{S: product pullbacks} that for any map $f \colon N \to M$ with $M$ boundary-less, there is a subcomplex of geometric cochains on $M$ that is transverse to $f$ and on which, consequently, pullbacks to $N$ are always defined.
We also consider Kronecker pairings of chains with cochains and homology with cohomology in \cref{S: Kronecker}.
This leads us to a universal coefficient theorem in \cref{S: UCT}.

In \cref{S: usual cup}, we apply Kreck and Singhof \cite{Krec10b} once again to say that the geometric cup product is abstractly isomorphic to the singular cohomology cup product.
...
In \cref{S: PD}, we show that the intersection map takes our tautological Poincar\'e duality of \cref{T: PD} precisely to classical Poincar\'e duality induced by cap product with the fundamental class.
\cref{S: umkehr} contains a short treatment of umkehr maps.
...
This is then applied to the cohomology of spherical link complements, including linking numbers in $S^3$ and the nontrivial triple linking of the Borromean rings.