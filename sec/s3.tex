...
\begin{proof}
	...
	\begin{tikzcd}
		...
		W \arrow[r, "g"] & M.
	\end{tikzcd}
	\]\greg{This is still in Tikzcd - should probably change to be consistent.}
	...
	For co-orientatability, it suffices by \cref{P: interior co-orientation} to consider the restriction to the interior of $V \times_M W$. Equivalently, we assume that $V$ and $W$ are manifolds for the remainder of the argument. 
...
\begin{definition}\label{D: pullback coorient}
	Suppose $f \colon V \to M$ and $g \colon W \to M$ are transverse with $f$ co-oriented and the normal bundle $\nu V$ of embedded image $e(V) \subset M \times \R^N$ given its Quillen normal orientation as defined in \cref{D: Quillen normal or}.
	As $\nu V$ is oriented, so is its pullback over $P = V \times_M W = (g \times \id_{\R^N})^{-1}(e(V)) \subset W \times \R^N$, which, by abuse of notation, we also label $\nu V$.
	Let $\beta_P$ and $\beta_W$ be local orientations of $P$ and $W$, let $\beta_{\nu V}$ denote the fiber orientation of the normal bundle $\nu V$ over $P$, and let $\beta_E$ be the standard orientation of $\R^N$.
...
\begin{proof}[Proof of \cref{L: pullback co well defined}]
	...
	In the construction involving $e$, if we choose $\beta_V$, $\beta_M$ so that $(\beta_V,\beta_M)$ is the co-orientation for $f$, then by the definition of the Quillen orientation, $\beta_{\nu V}$ will be such that $\beta_V \wedge \beta_{\nu V} = \beta_M \wedge \beta_E$ up to positive scalar.
	...
	Next, by the same argument by which embeddings such as $e_0$ and $e_1$ exist (see the proof of \cref{L: Quillen}), there is an embedding $\td H: V \times I \into M \times \R^N \times \R^Q$ for some $Q\geq 0$ so that if $\td \pi \colon M \times \R^N \times \R^Q \to M \times \R^N$ is the projection then $\td \pi \td H = H$.
...
Say that $h$ is a homotopy between the emebddings $h_0$ and $h_1$. 
...
\begin{align*}
	(V \times I) \times_M W &= \{ (x,t,y)\in V \times I \times M \mid \pi h(x,t) = g(y) \} \\
...
We can also think of these as co-orientations of the restrictions of $\pi h$ to each $V\times t \subset V \times I$. We co-orient the composite $\pi h \colon V \times I \to M$ with the co-orientation $(\beta_V \wedge \beta_I,\beta_M)$ at each $(x,t)$, with $\beta_I$ corresponding to the standard positive orientation of the interval.
...
\[(\beta_V \wedge \beta_I, \beta_V \wedge \beta_I \wedge \beta_{\nu(V \times I)}) * (\beta_M\wedge \beta_E,\beta_M) = (\beta_V\wedge \beta_I,\beta_M),\]
...
\[(\beta_V, \beta_V \wedge \beta_{\nu V_0})*(\beta_M \wedge \beta_E,\beta_M)=(\beta_V, \beta_M),\] i.e.\  $\beta_V \wedge \beta_{\nu V_0} = \beta_M \wedge \beta_E$. 
...
which is then $(\beta_P,\beta_W)$ by our preceding assumption that $\beta_P \wedge \beta_I \wedge \beta_{\nu(V \times I)}=\beta_W \wedge \beta_E$. 
