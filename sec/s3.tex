...
\begin{lemma}
Suppose that $f \colon V \to M$ and $g \colon W \to M$ are transverse with $f$ co-oriented. Let $\pi \colon M \times \R^N \to M$ be the projection, and let $h: V \times I \to \R^N$ be an embedding such that $\pi h(x,t) = f(x)$. Say that $h$ is a homotopy between the emebddings $e$ and $e'$. Then the co-orientations of $V \times_M W$ obtained using $e$ and $e'$ are the same. 
\end{lemma}
