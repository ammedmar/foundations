...
\subsubsection{Fiber products of immersions}\index{orientation!of immersions}
...
\begin{definition}\label{D: co-orientations}\index{co-orientations|textbf}
	...
	In particular, a co-orientation exists if and only if $\Hom(\Or(TW), \Or(g^*TM))$ is a trivial line bundle, in which case we say that $g$ is \textbf{co-orientable}.\index{co-orientable|textbf}
\end{definition}
...
Again, we call such a choice of basis vector around a point in $W$ a \textbf{local orientation}\index{orientation!local}, and, as for global orientations, often denote a local orientation by $\beta_W$.\index{$\beta_W$}
...
\begin{notation}\label{N: hat check}
	...
	If $W$ comes equipped with an orientation, we may write $\hat W$\index{$\hat W$} to refer to $W$ with the induced co-orientation on $g$.
	If $W$ (technically the map $g \colon W \to M$) comes equipped with a co-orientation, we may write $\check W$\index{$\check W$} to refer to $W$ with its induced orientation.
...
\begin{notation}\label{N: implicit notation}
	...
	In this, case we write $-V$\index{$-V$}\index{co-orienation!notation for opposite} to refer to the same map with the opposite co-orientation.
\end{notation}
...
\begin{proposition}\label{P: interior co-orientation}\index{co-orientability!only depends on map of interior}
...
\subsubsection{Co-orientations of boundaries of boundaries}\index{co-orientations!of $\bd^2$}
...
\subsection{Co-orientation of homotopies}\label{S: co-oriented homotopy}\index{co-orientation!of homotopies|(}
...
\begin{definition}\label{D: co-oriented homotopy}\index{co-orientation!co-oriented homotopy}
...
\begin{comment}
	...
	\end{proof}
\end{comment}
\index{co-orientation!of homotopies|)}
...
\begin{remark}\label{R: precise commutativity}\index{co-orientation!commutativity}
...
\begin{lemma}\label{L: co-orientable pullback}
	...
	Then:
	\begin{enumerate}
		\item\index{co-orientatability!of pullbacks} If $f$ is co-orientable, the pullback $f^* \colon P = V \times_M W \to W$ is co-orientable.
...
\begin{corollary}\index{co-orientability!of fiber products}
...
\begin{corollary}\label{C: co-orient W0}
	...
	Then $(\bd W)^0 = -\bd (W^0)$\index{$\bd (W^0)$ vs. $(\bd W)^0$} as co-oriented maps to $W$, with $W^0$ and $(\bd W)^0 = (gi_{\bd W})^{-1}(V)$ co-oriented as in \cref{P: codim 1 co-orient} as the pullbacks $V \times_M W \to W$ and $V \times_M \bd W \to W$.
\end{corollary}
...
Recall that we defined the oriented fiber product of oriented manifolds with corners in \cref{S: orientation of fiber products}.\index{external product!orientation}\index{orientation!of external product}
...
\begin{proposition}\label{P: oriented interchange}\index{orientation!of fiber product!of external products}\index{orientation!of external product!of fiber products}\index{criss cross}
...
\begin{lemma}\label{L: alternative transversality}\index{transversality!relation to the diagonal}\index{diagonal map!relation to transversality}\index{diagonal map}\index{$\diag$}
...
\begin{corollary}\label{C: cross is cup}\index{external product!as fiber product of pullbacks of projections}\index{fiber product!of pullbacks by projections is external product}\index{co-orientation!of external product!as fiber product of pullbacks of projections}\index{co-orientation!of fiber product!of pullbacks by projections is external product}
...
\begin{proposition}\label{P: cap cross}\index{cap orientation!of external product!}\index{external product!of fiber products with cap orientations}
...
To attempt to avoid confusion, let us consider $V$ and $W$ given as oriented, and we will write $V \times_M^o W$\index{$\times_M^o$} for the fiber product orientation of \cref{S: orientations} and $V \times_M^c W$\index{$\times_M^c$} for $(\hat V \times_M \hat W)\,\check{\vrule height1.3ex width0pt}$.
...
\begin{proposition}\label{P: compare exterior orientations}\index{external product!orientation vs. co-orientation}
