...
\greg{I've tried to add some text here explaining our symbolic conventions for working with orientations since otherwise I think it comes up pretty abruptly and is hard to understand. What do you think of this? Is it sufficient?\\
A. Differential form? I don't think we are considering sections on $\bigwedge^{m} T^\ast M$ (cotangent bundle). Maybe you just mean sections of $\bigwedge^{m} T^\ast M$. Maybe we can talk about this.}
...
Let us try to understand the orientation of $V \times_M W$ determined by orientations of $V$, $W$, and $M$ in this setting.\greg{Since this is early on and we haven't worked much with our form notation yet at this point, I tried to really improve the exposition in this section to make it much clearer. Please have a look and let me know what you think.}
...
\begin{example}\label{E: splitting example 1}
	...
	Its tangent space has trivial determinant line bundle, and we can choose the basis element to be the element $1 \in \bigwedge^0 T0 \cong \R$. If we give the normal bundle to $0$ in $\R$ the standard orientation in the positive direction, denoted by the standard basis vector $e_1$, then the corresponding normal orientation of the inclusion of $0$ into $\R$ will be $(1, 1 \wedge e_1) = (1, e_1)$.
...
\begin{example}\label{E: manifold decomposition}
	...
	Now, the normal bundle of $W^0$ in $W$ is the pullback via $\varphi g$ of the normal bundle of $0$ in $\R$. Giving the normal bundle of $0$ in $\R$ the standard orientation in the positive direction, denoted by the basis vector $e_1$, this orientation pulls back to a natural orientation of the normal bundle to $W^0$ in $W$. Applying \cref{normal co-or}, we obtain a normal co-orientation for $W^0$ in $W$, which we call the \textbf{co-orientation of $W^0$ in $W$ induced by $g\varphi$}. This is the co-orientation $(\beta_{W^0}, \beta_{W^0} \wedge (\varphi g)^*e_1)$.
...
As the product of two spaces is the same as their fiber product over a point, we have by equation \eqref{E: product boundary} and rearranging the order of components:
\begin{equation*}
	\bd(W \times I) =
...
\begin{proof}
	...
	For co-orientability, it suffices by \cref{P: interior co-orientation} to consider the restriction to the interior of $V \times_M W$. Equivalently, we assume that $V$ and $W$ are manifolds for the remainder of the argument.
	...
	\begin{equation}\label{D: pullback}
		\begin{tikzcd}
			P \arrow[r, "\pi_V"] \arrow[d] & V \arrow[d, "e"] \\
...
\begin{remark}\label{R: pullback representative 2}
	...
	In fact, there is a canonical map between this realization of $P$ and our standard realization $\{(v,w) \in V \times W \mid f(v) = g(w)\}$ given by $(v,(w,z)) \mapsto (v,w)$ with inverse given by $(v,w) \mapsto (v,(w,\pi_2(e(v))))$.

	We also already observed in the above proof that $P$ can be identified with $(g \times \id)^{-1}(e(V)) \subset W \times \R^N$.
	Of course when we think of $P$ as $\{(v,(w,z)) \in V \times (W \times \R^N) \mid e(v) = (g(w),z)\}$, the embedding into $W \times \R^N$ is just the map $(v,(w,z)) \mapsto (w,z)$.
\end{remark}
...
\begin{proof}
	...
	We may assume $V$ and $W$ are manifolds by \cref{P: interior co-orientation}, and then the map labeled $e^*$ is an embedding, as recalled in the proof of \cref{L: co-orientable pullback}. 
	We then note that it is equivalent to pull back the normal $\nu V$ to $X \times \R^N$ either in two steps or all at once.
	\[
	\begin{tikzcd}[column sep=large]
...
\begin{example}\label{E: V embedded}
	...
	In this case, we know from \cref{S: normal orientation} that a co-orientation is equivalent to an orientation $\beta_{\nu V}$ of the normal bundle to $V$ in $M$.
...
\begin{proof}
	It suffices to demonstrate this property in the neighborhood of any intersection point, so we may assume that $f$ and $g$ are embeddings of manifolds and consider $x \in V$, $y \in W$ with $f(x) = g(y) = z \in M$.
...
\begin{lemma}\label{L: pullback boundary normal}
	...
	In this case we know that $V \times_M W = g^{-1}(V)$ and the pullback of the normal bundle $\nu V$ of $V$ in $M$ is the normal bundle of $g^{-1}(V)$ in $W$, and similarly replacing $\bd V$ with $V$. 
	Let $w\in \bd (V \times_M W) = (\bd V) \times_M W$. 
...
\end{lemma}
\begin{proof}
Let $v = g(w)$, and let $a \in T_v V \subset T_v M$ such that $a \notin T_v (\bd V)$. As $g$ is transverse to $\bd V$, there are vectors $b \in T_w W$ and $c \in T_v(\bd V)$ such that $a= Dg(b) + c$. Now rewriting as $Dg(b) = a - c$, the righthand side is contained in $T_v(V)$, but not in $T_v(\bd V)$ or else $a$ would be in $T_v(\bd V)$. As the tangent space of the pullback $g^{-1}(V)$ at $w$ is the pullback of the tangent spaces of $T_v V$ and $T_w W$ mapping to $T_v M$ by \cref{L: tangent of pullbacks}, we see that the pair $(a-c, b) \in T_v V \times_{T_v M} T_w W$ is in the tangent space of the pullback, and as the embedding $V \times_M W \to W$ corresponds to the projection in the $W$ co-ordinate, we see that $b$ must be a tangent vector to $g^{-1}(V)$ mapping to $Dg(b)$ via $Dg$ as desired. Note that $b$ cannot be in $T_w((\bd V) \times_M W)$ as such a vector would map to $T_v (\bd V)$. It is clear that $Dg$ must take inward pointing vectors to inward pointing vectors. 
\end{proof}
...
\begin{proof}
	...
	By \cite[Sections 1.3-1.4]{GuPo74}, we may also choose the charts so that $Df_x(e_i) = e_i$ for $1 \leq i \leq k$ for some $k<\dim(V) = v$ and $Df_x(e_i) = 0$ for $i>k$.
	...
	Finally, as $f$ and $g$ are proper and transverse, there is an $\epsilon$ so that $H(-,t)$ and $g$ will be transverse for $0 \leq t \leq \epsilon$ by stability of transversality\footnote{See \cite[Theorem 1.6]{GuPo74} for the case where one manifold is compact and the other is embedded. A proof for the current situation is given within our proof of \cref{T: transverse complex}, below.}.
...
\begin{proof}[Proof of \cref{P: graded comm}]
	...
	Then $(V \times I) \times_M W$ will have its fiber product co-orientation, and by \cref{leibniz} two of its boundary components will be (topologically) $f \times_M g \colon (V \times 0) \times_M W \to M$ and
...
\begin{example}\label{E: codim 0 and 1 co-or as fiber products}
	...
	In particular, $M^\pm$ are embedded codimension $0$ manifolds with corners in $M$ and $\bd M^\pm = M^0$.
	...
	Then let $W^\pm \defeq g^{-1}(M^\pm) = M^\pm \times_M W$, and by \cref{P: codim 0 pullback}, the pullback co-orientations of $W^\pm \into W$ are again the tautological co-orientations of the embeddings.
...
\begin{proposition}\label{P: codim 1 co-orient}
	...
	Suppose the embedding $f \colon V \into M$ is co-oriented by the normal co-orientation $(\beta_V, \beta_V \wedge \beta_\nu)$.
	Let $W$ be a manifold without corners and suppose $g \colon W \to M$ is transverse to $f$ so that $W^0 \defeq g^{-1}(V)$ is a codimension $1$ submanifold with corners with oriented pullback normal vector bundle $\nu_0$.
	Then the co-oriented pullback $V \times_M W \to W$ is the embedding $W^0 \into W$, co-oriented by $(\beta_P, \beta_P \wedge \beta_{\nu_0})$.
\end{proposition}

\begin{proof}
	It is again standard that the pullback is $g^{-1}(V) = W^0$, and it is a codimension $1$ manifold with corners by \cite[Theorem 6.4]{Joy12}.
