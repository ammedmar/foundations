...
A great benefit to such thinking is that the classical operations of algebraic topology, such as cup and cap products, can be described \textit{at the level of chains and cochains} by simple geometric operations based on intersection (more generally, fiber product), without recourse to chain approximations to the diagonals, Alexander--Whitney maps, or other such combinatorics.
...
Loosely speaking, no fully-defined integral cochain construction computing ordinary cohomology can be made canonically into a (graded) commutative ring.
...
\textbf{Above the foundation.} Further developments are already in process, including work on partial algebra structures and bar complexes for geometric cochains in \cite{GBF47} and applications of geometric cochains and their bar complexes to concordance invariants of links in \cite{FGKSW}.
Other aspects of geometric homology and cohomology remain to be explored.
We have yet to identify a satisfactory theory of geometric (co)homology of pairs, and we have not developed transverse products on manifolds with boundaries or corners.
Consequently, we do not yet have a Lefschetz Duality theorem.
We here treat coefficients other than $\Z$ only minimally; for example, we have not considered a cap product with coefficients, and in some places we consider only $\Z$, $\Q$, and $\Z_n$ as coefficient rings.
There are also finite generation conditions for some theorems that we suspect are not necessary but do not know how to avoid.
In other places, we would like to remove certain properness assumptions for maps between manifolds.
As indicated in the title, we hope here to provide the foundations on which to build a much larger edifice.
...
Rather than considering algebraic duals of geometric chains, geometric cochains are again represented by maps from manifolds with corners, however instead of the condition that the domain manifold be compact and oriented, rather the map must be proper and co-oriented.
...
Where our theory goes further is that it provides a well-defined partial commutative algebra structure \cite{McC} (equivalently, a Segal $\Gamma$ structure in chain complexes\footnote{Our adoption of $\Gamma$ in our notation for geometric chains and cochains was not made with Segal's category $\Gamma$ in mind, but it turns out to be a happy coincidence.} \cite{Se74}) on the geometric cochains themselves.
...
Given maps $f \colon V \to M$ and $g \colon W \to M$, we typically write ``pullback'' for the resulting object $V \times_M W$ over $W$ and ``fiber product'' for the object $V \times_M W$ over $M$.
See \cref{S: pullbacks} for details. \index{pullback!vs. fiber product}\index{fiber product!vs. pullback}
