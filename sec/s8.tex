...
\subsection{Alexander Duality}\index{Alexander Duality|(}
...
\end{remark}\index{Alexander Duality|)}


\subsection{Applications to spherical links}\label{S: links}\index{Alexander Duality!application to links|(}
...
Let $L$ be a link of two components, $L_1$ and $L_2$, in $S^3$, and let $S_1$ and $S_2$ be corresponding Seifert surfaces that are transverse to each other.
...
We claim that, using this choice of generator to identify $H^2_\Gamma(S^3-L)$ with $\Z$, the cup product $\underline{\mr S_1} \uplus \underline{\mr S_2}$ represents the linking number of $L_1$ and $L_2$, where we use the second (of eight!) characterizations of linking numbers given by Rolfsen in \cite[Section 5D]{Ro90}.\index{linking number}
...
\subsubsection{A triple linking number}\index{triple linking number|(}\index{link!triple linking number|(}
...
Similarly, for $S_{23}$, consider the intersection of $S_3$ with the half space $y \geq 0$.
...
\index{Alexander Duality!application to links|)}
\index{triple linking number|)}\index{link!triple linking number|)}
...
Furthermore, in $S^3$ we know from \cref{leibniz} that
$$\bd (S_1 \times_{S^3} S_2) = ((\bd S_1) \times_{S^3} S_2 ) \sqcup (S_1 \times_{S^3} \bd S_2) = (L_1 \times_{S^3} S_2) \sqcup (S_1 \times_{S^3} L_2),$$
where here we continue to treat $L_1$ and $L_2$ as co-oriented as the boundaries of the co-oriented $S_1$ and $S_2$.



UNNECESSARY??: Again considering \cref{P: normal pullback}, the normal co-orientation of $L_1 \times_{S^3} S_2$ is $\beta_{\nu_1} \wedge \beta_{\mu_1} \wedge \beta_{\nu_2}$, and the normal co-orientation of $S_1 \times_{S^3} L_2$ is $\beta_{\nu_1} \wedge \beta_{\nu_2} \wedge \beta_{\mu_2}$.




---------
If we let $\beta_{L_i}$ be the orientation of $L_i$ and $\nu_{S_i}$ the normal to $L_i$ pointing into $S_i$, then by \cref{D: boundary co-orientation} the standard co-orientation of the inclusion $L_i \into S_i$ is $(\beta_{L_i}, \beta_{L_i} \wedge \beta_{\nu_{S_i}}$.

So the co-orientation for $L_i$ into $S_i$ is $(\beta_{L_i}, \beta_{S_i})$ and so the co-orientation of $L_i$ into $S^3$ as the boundary of $S_i$ is $$(\beta_{L_i}, \beta_{S_i}) * (\beta_{S_i}, \beta_{S^3}) = (\beta_{L_i}, \beta_S^3) = (\beta_{L_i}, \beta_{S_i} \wedge \beta_{\nu_i}) = (\beta_{L_i}, \beta_{L_i} \wedge \beta_{\nu_{S_i}} \wedge \beta_{\nu_i}).$$
It follows that $(L_1 \times_{S^3} S_2 )$ is normally co-oriented by $\beta_{\nu_{S_1}} \wedge \beta_{\nu_1} \wedge \beta_{\nu_2}$, while $S_1 \times_{S^3} L_2$ is normally co-oriented by $(\beta_{\nu_1} \wedge \beta_{\nu_{S_2}} \wedge \beta_{\nu_2}$.

In particular, in the first case, if the direction of $L_1$ agrees with the positive orientation of the binormal to $S_2$ then the normal orientation becomes $\beta_{\nu_{S_1}} \wedge \beta_{\nu_1} \wedge \beta_{\nu_{S_2}}$



Now let $C$ be a component of $\mr S_1 \cap \mr S_2$.
We have the following possibilities:

\begin{enumerate}
\item $C$ is a closed curve.
In this case it comes from the interior of $S_1$ intersecting the interior of $S_2$.
This curve does not contribute to $H^2_\Gamma(S^3-L)$ nor to the linking number.

\item $C$ runs from $L_2$ to $L_2$.
In this case, $C$ arises from the intersection of $S_2$ with the interior of $S_1$ and the intersection includes two points of $L_2$; in particular, the intersection does not include any points of $L_1$.
There are no contributions to the linking number and also this curve does not contribute to $H^2_\Gamma(S^3-L)$.

\item $C$ runs from $L_1$ to $L_1$.
In this case,  $C$ arises from the intersection of $S_2$ with the interior of $S_1$,
