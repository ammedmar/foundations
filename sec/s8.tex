...
\begin{proof}
	...
	As $V$ and $V'$ represent the same cohomology class, by \cref{R: cycles and boundaries} there exists $Z \in PC^*_\Gamma(S^n-L)$ so that $\bd Z\sqcup -V \sqcup V' \in Q^*(S^n-L)$; we may also assume $Z$ transverse to $S_x$ by REFERENCE NEEDED\greg{This needs a reference after editing Chapter 7 - can be pulled out of proof of 7.22 and 7.24 as of Feb 3, 2025 version.}.
...
\begin{proof}
	...
	By REFERENCE NEEDED\greg{This needs a reference after editing Chapter 7 - can be pulled out of proof of 7.22 and 7.24 as of Feb 3, 2025 version.} we may suppose $Z$ is transverse to $S_x$, and then $\mc A \mc Z(\uV)$ is represented by $\pi(\mr Z \times_{S^n} S_x) = \pi(\hat Z \times_{S^n} S_x)$, which we denote $W$.
