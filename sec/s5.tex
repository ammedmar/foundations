...
\begin{proposition}\label{P: cohomology pullback}\index{geometric homology!functoriality}\index{geometric cohomology!functoriality}\index{geometric homology!functoriality!homotopy}\index{geometric cohomology!functoriality!homotopy}
...
\begin{definition}\label{D: connecting}\index{Mayer-Vietoris sequence!for geometric cohomology!connecting morphism|textbf}\index{Mayer-Vietoris sequence!for geometric cohomology!connecting morphism|(}\index{connecting morphism for geometric cohomology|(}
...
\begin{example}\index{geometric cohomology!of the projective plane}
...
Rather than maps of simplices $\Delta^k \to M$, the cubical singular chain complex,\index{cubical singular chains} which we denote $SK_*(M)$,\index{$SK_*(M)$} is generated by maps $\interval^k \to M$\index{$\interval^k$} with $\interval$ being the standard interval $\interval = [0,1]$ (it will be convenient henceforth to use this notation for the interval in this context).
...
The homology of the chain complex $SK_*(M)$ is not isomorphic to singular homology, as it does not satisfy the dimension axiom, so one instead forms the normalized complex $NK_*(M)$\index{$NK_*(M)$}\index{cubical singular chains!normalized} by quotienting out the subcomplex of \textit{degenerate singular cubes} that is generated by singular cubes $\sigma \colon \interval^k \to M$ such that $\sigma$ does not depend on at least one of the variables.
...
\index{smooth singular chains|)}

\subsection{Appendix: smooth singular cubes}\label{S: smooth cubes}\index{smooth singular cubes|(}
...
\begin{definition}\index{geometric chains!with coefficients|textbf}\index{geometric homology!with coefficients|textbf}
