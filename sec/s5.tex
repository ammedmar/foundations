...
\begin{definition}\label{D: connecting}
	Suppose $U, V, U \cap V \subset M$ are non-empty open subsets of a manifold without boundary and $\uW \in H^k_\Gamma(U \cap V)$ is represented by $r_W \colon W \to U \cap V$.
	Of course we can also consider $r_W$ to have image in $U \cup V$.
	Let $\phi \colon U \cup V \to [-1/2,1/2]$ separate $W$ over $U$ and $V$.
	Define $\delta(\uW) \in H^{k+1}_\Gamma(U \cup V)$ to be represented by $-W^0 = -(\phi r_W)^{-1}(0)$, which we recall can be identified with $-\phi^{-1}(0)\times_{U \cup V} W$ by \cref{L: pm0 as fiber products}.
\end{definition}
...
So for $p \in (-1/2,1/2)$, we let $M^p = \phi^{-1}(p)$ and $W^p = (\phi r_W)^{-1}(p) \cong M^p \times_{U \cup V} W$.
Note that as $\phi$ separates $U$ and $V$, we have $M^p \subset U \cap V \subset U \cup V \subset M$ for all $p \in (-1/2,1/2)$, so the space $M^p$ is unambiguous whether we consider the domain of $\phi$ to be $U \cup V$ or $U \cap V$.

We first show our construction does give an element of $H^*_\Gamma(U \cup V)$.

\begin{lemma}
	Suppose $U, V, U \cap V \subset M$ are non-empty open subsets of a manifold without boundary and $\uW \in H^k_\Gamma(U \cap V)$ is represented by $r_W \colon W \to U \cap V$.
	Consider $r_W$ to have image in $U \cup V$.
	Let $\phi \colon U \cup V \to [-1/2,1/2]$ separate $U$ and $V$, and let $p \in (-1/2, 1/2)$ be a regular value for $\phi$ and $\phi r_W$.
	Then $W^p = M^p \times_{U \cup V} W$ is an element of $PC^*_\Gamma(U \cup V)$ with boundary in $Q^*(U \cup V)$, so it represents an element of $H^*_\Gamma(U \cup V)$.
\end{lemma}

\begin{proof}
	For simplicity and consistency with the notation in \cref{S: codim 0 and 1 co-or,S: splitting}, we take $p = 0$, but the same argument works for any $p \in (-1/2, 1/2)$ satisfying the hypotheses. By \cref{L: W0 cochain}, $M^0 \times_{U\cap V} W$ is an element of $PC^*_\Gamma(U \cap V)$, and this provides a co-orientation of $M^0 \times_{U \cap V} W \to U \cup V$, but we must show that the resulting map to $U \cup V$ is proper.

	We may consider $r_W$ as mapping to $U \cup V$ by composing with the inclusion $U \cap V \into U \cup V$ with its tautological co-orientation, although the composite $W \to U \cup V$ may no longer be proper.
	By assumption, $0$ is a regular value of $\phi$ and $\phi r_W$, and we recall from \cref{E: manifold decomposition,L: pm0 as fiber products} that this implies $r_W$ is transverse to the embedded submanifold $M^0 \subset U \cup V$ and allows us to identify $W^0$ with $M^0 \times_{U \cup V} W$.

	Next we show that this fiber product $W^0 \to U \cup V$ is proper.
	The space $M^0$ is a closed subset of $U \cup V$, so its embedding into $U \cup V$ is proper.
	By \cref{L: co-orientable pullback}, the pullback $W \times_{U \cup V} M^0 \to \phi^{-1}(0)$ is proper.
	As the composition of proper maps is proper, the composite  $W \times_{U \cup V} M^0 \to \phi^{-1}(0) \to U \cup V$ is proper.
	But this is the fiber product $W \times_{U \cup V} M^0 \to U \cup V$, which is symmetric topologically, so $W^0 \cong M^0 \times_{U \cup V} W \to U \cup V$ is proper.
	We co-oriented $W^0 \to U \cup V$ as a fiber product as in \cref{L: W0 cochain}, so we have $W^0 \in PC^*_\Gamma(U \cup V)$.

	Finally, as $M^0$ has no boundary, we compute using \cref{leibniz} that
	$$\bd (W^0) = - M^0 \times_{U \cup V} \bd W.$$
	But $W$ represents a cocycle in $U \cap V$, so $\bd W$ is a union of trivial and degenerate precochains in $U \cap V$.
	The definitions of trivial and degenerate do not rely on the properness of the map, so $\bd W$ will also be a union of trivial and degenerate manifolds over $U \cup V$.
	Consequently, the same is true of $M^0 \times_{U \cup V} \bd W$ by \cref{L: pullback with Q}, noting the proof of that result does not rely on any map being proper.
\end{proof}
...
\begin{lemma}\label{L: different point}
	...
	Then $W^p$ and $W^q$, as defined just above, represent the same element of $H^*_\Gamma(U \cup V)$.
\end{lemma}

\begin{proof}
	Suppose without loss of generality that $p<q$.
	Then the submanifold $\phi^{-1}([p,q]) \subset U \cap V \subset U \cup V$ will be transverse to $r_W \colon W \to M$.
	...
	Thus, as in \cref{L: Wpq cochain}, $$\bd(\varphi^{-1}([p,q])\times_{U \cup V} W = (M^p \times_{U \cup V} W)\sqcup -(M^q \times_{U \cup V} W) \sqcup (\phi^{-1}([p,q])\times_{U \cup V} \bd W).$$
	As $W$ is a cocycle, the last term is in $Q^*(U \cup V)$ (with support in $U \cap V$) by \cref{L: pullback with Q}, so $M^p \times_{U \cup V} W$ and $M^q \times_{U \cup V} W$ represent cohomologous cocycles.
\end{proof}
...
\begin{proof}
	...
	As in the proofs of the preceding lemmas, the fiber product with $M^p$ over $U \cup V$ takes an element of $PC^*_\Gamma(U \cap V)$ to $PC^*_\Gamma(U \cup V)$ and an element of $Q^*(U \cap V)$ to $Q^*(U \cup V)$.
	So
	$$M^p \times_{U \cup V}(\bd Z \sqcup -W_1 \sqcup W_2) = (\bd Z)^p \sqcup -W_1^p \sqcup W_2^p$$
	is an element of $Q^*(U \cup V)$.
	Furthermore, by an obvious modification to \cref{C: co-orient W0}, we have $\bd (Z^p) = -(\bd Z)^p$.
	So  $$-\bd (Z^p) \sqcup -W_1^p \sqcup W_2^p \in Q^*(U \cup V),$$
	which implies that $W_1^p$ and $W_2^p$ represent the same cohomology class in $U \cup V$.
\end{proof}
...
\begin{proof}
	...
	We must show that $\phi_1^{-1}(0)\times_{U \cup W} W_1$ and $\phi_2^{-1}(0)\times_{U \cup W} W_2$ represent the same element of $H^*_\Gamma(U \cup V)$.
	...
	we have $\Phi^{-1}(p_1) \times_{U \cup V} W \cong \phi_1^{-1}(p_1) \times_{U \cup V} W$ and similar for $p_2$ and $\phi_2$.
	So now  $\Phi^{-1}(p_1) \times_{U \cup V} W$ and  $\Phi^{-1}(p_2) \times_{U \cup V} W$ represent the same element in $H^*_\Gamma(U \cup V)$ by \cref{L: different point}.
