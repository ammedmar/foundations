...
The \textbf{reduced geometric homology} $\td H_*^\Gamma(M)$ of the manifold without corners $M$ is defined to be the kernel of the homormorphism $H_*^\Gamma(M) \to H_*^\Gamma(pt)$ induced by the unique map of $M$ to a point.
...
The functoriality of geometric cohomology allows us to introduce reduced cohomology; cf.\ \cite[page 240]{Span81}.
...
So once we have shown in \cref{T: geometric is singular} that $H^*_\Gamma(-) \cong H^*(-;\Z)$ as functors, it will follow that for any $M$ we have $H^0_\Gamma(M) \cong \prod \Z$, where the product is indexed over the components of $M$ with each factor generated by the identity map of that component with its tautological co-orientation.
