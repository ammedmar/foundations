...
\begin{definition}\label{D: cohomology pullback and homology transfer}
	...
	Then we define $f^*(\uW) \in H^*_\Gamma(M)$ as the cohomology class represented by the pullback $W \times_N M \to M$.
	If $M$ is oriented, $f$ is proper, and $\uW \in H_*^\Gamma(N)$ is represented by $r_W \colon W \to N$, then we also take $g$ to be proper (also by \cref{T: basic trans}), in which case we define $f^*(\uW) \in H_*^\Gamma(M)$ as the homology class represented by the pullback $W \times_N M \to M$.
\end{definition}
...
\begin{proof}
	...
	Let $G'$ be a smooth homotopy from $g_0$ to $g_1$, which exists by \cite[Proposition 9.2.33]{MaDo92}  (or, if $f$ is proper and $g_0$ and $g_1$ are smooth maps properly homotopic to $f$, by \cite[Proposition 9.2.35]{MaDo92}).
	By \cref{T: homotopy trans} there exists a smooth homotopy $G \colon M \times I \to N$ from $g_0$ to $g_1$ such that $G$ is transverse to $r_W$; in particular we apply \cref{T: basic trans} with the map $f$ of that theorem being our smooth homotopy $G'$ here from $g_0$ to $g_1$, and then the resulting map $h(-,1)$ of that theorem will be our desired $G$, while the restriction of $G = h(-,1)$ to $\bd (M \times I) = M \times \{0,1\}$ remains $g_0 \sqcup g_1$. 
	...
	So $\bd W \times_N (M \times I) \to M \times I$ is in\footnote{Note that even if $M$ is without boundary, $M \times I$ will have boundary (unless $M$ is empty).
		...
		Thus we obtain an element of $Q^*(M)$ whether one prefers to define $PC_\Gamma^*(-)$ in general using smooth or weakly smooth maps.} $Q^*(M \times I)$ by \cref{L: pullback map Q}, and it is then easy to see that the projection to $M$ preserve triviality and small rank.
