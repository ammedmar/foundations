...
Although we use $\phi^{-1}(0)$ in our construction of $\delta$, it will be useful in our arguments to allow our regular value to be anywhere in $(-1/2,1/2)$, and we immediately work in this generality.


\begin{lemma}
	Suppose $U, V, U \cap V \subset M$ are non-empty open subsets of a manifold without boundary and $\uW \in H^k_\Gamma(U \cap V)$ is represented by $r_W \colon W \to U \cap V$.
	Consider $r_W$ to have image in $U \cup V$.
	Let $\phi \colon U \cup V \to [-1/2,1/2]$ separate $U$ and $V$, and let $p \in (-1/2, 1/2)$ be a regular value for $\phi r_W$.
	Then $W^p = \phi^{-1}(p) \times_{U \cup V} W$ is an element of $PC^*_\Gamma(U \cup V)$ with boundary in $Q^*(U \cup V)$, so it represents an element of $H^*_\Gamma(U \cup V)$.
\end{lemma}

\begin{proof}
	For simplicity and consistency with the notation in \cref{S: codim 0 and 1 co-or}, we take $p = 0$, but the same argument works for any regular value $p \in (-1/2, 1/2)$.

	We first note that we may consider $r_W$ as mapping to $U \cup V$ by composing with the inclusion $U \cap V \into U \cup V$, although $W \to U \cup V$ may no longer be proper.
	By assumption, $0$ is a regular value of $\phi r_W$, and we recall from \cref{E: manifold decomposition} that this implies $r_W$ is transverse to $\phi^{-1}(0)$ and allows us to identify $W^0$ with $\phi^{-1}(0) \times_{U \cup V} W$.

	Next we show that the map $W^0 \to U \cup V$ is proper.
	The space $\phi^{-1}(0)$ is a closed subset, so its embedding into $U \cup V$ is proper.
	By \cref{L: co-orientable pullback}, the pullback $W \times_{U \cup V} \phi^{-1}(0) \to \phi^{-1}(0)$ is proper.
	As the composition of proper maps is proper, the composite  $W \times_{U \cup V} \phi^{-1}(0) \to \phi^{-1}(0) \to U \cup V$ is proper.
	But this is the fiber product $W \times_{U \cup V} \phi^{-1}(0) \to U \cup V$, which is symmetric topologically, so $W^0 = \phi^{-1}(0) \times_{U \cup V} W \to U \cup V$ is proper.

	We co-oriented $W^0 \to U \cup V$ as in \cref{E: codim 1 pullbacks} in \cref{S: codim 0 and 1 co-or}, so we have a precochain.
	Finally, as $\phi^{-1}(0)$ has no boundary, we compute using \cref{leibniz} that
	$$\bd W^0 = \pm \phi^{-1}(0) \times_{U \cup V} \bd W,$$
	which will be proper in $U \cup V$ for the same reasons as for $W^0$.
	...
	Consequently, the same is true of $\phi^{-1}(0) \times_{U \cup V} \bd W$ by \cref{L: pullback with Q}, noting the proof of that result does not rely on $V$ or $W$ being proper.
\end{proof}
...
\begin{lemma}\label{L: different point}
	...
	Let $p, q\in (-1/2,1/2)$ be any points such that $\phi$ and $\phi r_W$ are transverse to the inclusions of $p$ and $q$ into $(-1/2,1/2)$, and let $W^p$ and $W^{q}$ be the fiber products $\phi^{-1}(p) \times_M W$ and $\phi^{-1}(q) \times_{U \cup V} W$ considered as elements of  $PC^*_\Gamma(U \cap V)$.
