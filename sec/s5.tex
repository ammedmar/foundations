...
\begin{theorem}\label{T: homology homotopy functor}
	...
	In other words:
	\begin{enumerate}
		\item If $\id \colon M \to M$ is the identity map with its tautological co-orientation, then $\id_*$ is the identity map on $H_*^\Gamma(M)$ and $H^*_\Gamma(M)$.
...
\begin{corollary}
	...
	In other words:
		\begin{enumerate}
			...
			\item  If $f_1 \colon M \to N$ and $f_2 \colon N \to X$ are continuous maps of manifolds without boundary, then $(f_2f_1)^* = f_1^*f_2^* \colon H^*_\Gamma(X) \to H^*_\Gamma(M)$; if $f_1$ and $f_2$ are also proper maps and $M$ and $N$ are oriented, then $(f_2f_1)^* = f_1^*f_2^* \colon  H_*^\Gamma(X) \to H_*^\Gamma(M)$.
		\end{enumerate}
\end{corollary}
\begin{proof}
	We first consider the map on cohomology.
...
\begin{proof}
	...
	We observe that $(U \cap V)^+ = \phi^{-1}([0,1/2])\cap(U \cap V)$ is a closed subspace of both $U \cap V$ and of $U$, as $\phi^{-1}([0,1/2]) \cap (U \cap V) = \phi|_U^{-1}([0,1/2])$ given that $\phi(U\setminus V) = -1/2$.
	...
	Similarly, there is no $W^+$ in $PC^*_\Gamma(V)$.
	...
	Similarly, we have $\phi^{-1}([-1/2,0]) = \phi|_U^{-1}([-1/2,0])$, and so $\phi^{-1}([-1/2,0])$ is a closed subset of $U$ and of $U \cup V$.
	Therefore, the inclusions $\phi^{-1}([-1/2,0]) \into U$ and $\phi^{-1}([-1/2,0]) \into U \cup V$ are proper so that if $W \in PC^*_\Gamma(U)$, then $W^-$ is well-defined in both $PC^*_\Gamma(U)$ and $PC^*_\Gamma(U \cup V)$.
	See \cref{F: MV2}.
	Analogously, if $W \in PC^*_\Gamma(V)$ then we obtain $W^+$ in both $PC^*_\Gamma(V)$ and $PC^*_\Gamma(U \cup V)$.
	...
	We also note that $\phi^{-1}(0)$ is closed in $U \cap V$, $U$, $V$, and $U \cup V$, so for any $W$ in $PC_\Gamma^*(-)$ for any of these spaces, $W^0$ is also in $PC^*_\Gamma(-)$ for all these spaces.

	Finally, we note that if $W \in Q^*(U \cap V)$ is also well defined as an element of $PC^*(U)$, then $W \in Q^*(U)$ as, directly from the definitions, being trivial or degenerate with image in $U \cap V$ implies being trivial or degenerate in $U$.
	The same consideration holds replacing $U$ with $V$ or for the inclusions of $U$ and $V$ into $U \cup V$.
	...
	As $W^0$ already maps to $U \cap V$, we can write simply $(-W^0, W^0)$.
	...
	Now $\bd W \in Q^*(U \cap V)$ as $W$ is a cocycle, so $(\bd W)^+ \in Q^*(U \cap V)$ by \cref{C: creasing Q} and so also $(\bd W)^+ \in Q^*(U)$.
	Passing to chains, we thus have $\bd \underline{W^+} = \underline{W^0}$ in $C^*_\Gamma(U)$, so $W^0$ represents $0 \in H^k_\Gamma(U)$.
	...
	So by \cref{R: cycles and boundaries} there exists $A \in PC^*_\Gamma(U)$ with $\bd A \sqcup - W|_U \in Q^*(U)$ and $B \in PC^*_\Gamma(V)$ with $\bd B \sqcup - W|_V \in Q^*(V)$.
	...
	But $\underline{W^-} + \underline{W^+}$ is cohomologous to $\uW$ by creasing, \cref{T: cohomology creasing}, so $W$ is cohomologous to $\underline{A^0}-\underline{B^0}$, which represents $\delta(\underline{B}|_{U \cap V}-\underline{A}|_{U \cap V})$ in $H^*_\Gamma(U \cup V)$.
	...
	We choose $\phi$ to be separating point for $Z$, $W_1$, and $W_2$.
	...
	\end{align*}
	As $W_1$ and $W_2$ are cycles in $U$ and $V$ respectively, $(\bd W_1)^-$ and $(\bd W_2)^+$ are in $Q^*(U)$ and $Q^*(V)$ by \cref{R: cycles and boundaries,C: creasing Q}, and so they are also in $Q^*(U \cup V)$.
	...
	But $(W_1|_{U \cap V})^0 = W_1^0$ and similarly for $W_2$.
	...
	\[\bd (Z^+) \sqcup \gamma|_U = Z^0 \sqcup (\bd Z)^+ \sqcup W_1^- \sqcup -Z^0 \sqcup -(W_2^+),\]
	using that the components of $\gamma$ are already precochains in both $U \cap V$ and $U$.
	So passing to chains and undertaking some further manipulations, we have in $C^*_\Gamma(U)$ that
	\begin{align*}
	\bd (\underline{Z^+}) + \underline{\gamma|_U} &
		...
		&= \underline{(\bd Z)^+} + \underline{W_1^-}  -\underline{W_2^+} + \underline{W_1^+} - \underline{W_1^+}\\
		&= \underline{(\bd Z \sqcup - W_1|_{U \cap V} \sqcup - W_2|_{U \cap V})^+}  + \underline{W_1^-}   + \underline{W_1^+}\\
		&=\underline{W_1^-}   + \underline{W_1^+}\\
		&=\underline{W_1}.
	\end{align*}
	Here we have used that $\bd Z \sqcup - W_1|_{U \cap V} \sqcup - W_2|_{U \cap V} \in Q^*(U \cap V)$, which implies that $(\bd Z \sqcup - W_1|_{U \cap V} \sqcup - W_2|_{U \cap V})^+ \in Q^*(U)$, employing \cref{C: creasing Q}, and that $\underline{W_1^-}   + \underline{W_1^+}= \underline{W_1}$ in $H^*_\Gamma(U)$ by creasing, \cref{T: creasing}.
	So $\underline{W_1} = \underline{\gamma|_U} = \underline{\gamma}|_U$, as claimed.

	Similarly, in $V$ we have
	\begin{align*}
	\bd (\underline{Z^-}) - \underline{\gamma|_V} &
		= - \underline Z^0 + \underline{(\bd Z)^-} - \underline{W_1^-} + \underline Z^0 +\underline{W_2^+|_V} + \underline{W_2^-} - \underline{W_2^-}\\
		&= \underline{(\bd Z)^-} - \underline{W_1^-}  + \underline{W_2^+} + \underline{W_2^-} - \underline{W_2^-}\\
		&= -\underline{(\bd Z \sqcup + W_1|_{U \cap V} \sqcup + W_2|_{U \cap V})^-}  + \underline{W_2^-}   + \underline{W_2^+}\\
		&=\underline{W_2^-}   + \underline{W_2^+}\\
		&=\underline{W_2}.
	\end{align*}
	So $\underline{W_2} = -\underline{\gamma|_V} = -\underline{\gamma}|_V$.
	...
	By the discussion above, $W^- \in PC^*_\Gamma(U \cup V)$, and $$\bd (W^-) = -W^0 \sqcup (\bd W)^-.$$ As $W$ is a cocycle, $\bd W \in Q^*(U)$, and so $(\bd W)^-$ is in $Q^*(U)$ and $Q^*(U \cup V)$ via \cref{C: creasing Q}.
	...
	\end{align*}
	The cochain $\underline{W^+} + \underline{W^-}$ represents $\uW \in H^*_\Gamma(U \cap V)$ via creasing, \cref{T: cohomology creasing}.
\end{proof}
...
\begin{proof}
	...
	Axiom \ref{I: homotopy functor} holds from the definitions and \cref{T: homology homotopy functor,P: cohomology pullback}.
