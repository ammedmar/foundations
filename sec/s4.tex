...
\begin{lemma}\label{L: product preserves iso}
	...
	In particular, the isomorphism classes of the fiber products depend only on the isomorphism classes of the inputs.
\end{lemma}
\begin{proof}
	...
	\begin{diagram}\label{E: fiber orientation}
		0 &\rTo & TP & \rTo^{D\pi_V \oplus D\pi_W} & \pi_V^*(TV) \oplus \pi_W^*(TW) & \rTo^{\pi_V^*(Df)-\pi_W^*(Dg)} & (f\pi_V)^*TM &\rTo 0\\
		&& \dTo &&\dTo && \dTo \\
		0 &\rTo & TP' & \rTo^{D\pi_{V'} \oplus D\pi_{W'}} & \pi_{V'}^*(TV') \oplus \pi_{W'}^*(TW') & \rTo^{\pi_{V'}^*(Df')-\pi_{W'}^*(Dg')} & (f'\pi_{V'})^*TM & \rTo 0.
	\end{diagram}
...
Given the preceding lemma and remark, from here on we will usually not explicitly distinguish between maps (co\nobreakdash-)oriented maps $V \to M$ and their isomorphism classes when discussing products.
...
\begin{proof}
	...
	If $\rho$ is a (co\nobreakdash-)orientation reversing diffeomorphism of $V$ over $M$, then $\rho \times_M \id_W$ is a (co\nobreakdash-)orientation reversing diffeomorphism of $V \times_M W$ by \cref{R: product preserves reverse}.
...
\begin{proof}
	...
	If $\rho$ is a (co\nobreakdash-)orientation reversing diffeomorphism of $V$ over $M$, then $\rho \times \id_W$ is a (co\nobreakdash-)orientation reversing diffeomorphism of $V \times_M W$.
...
By \cite[Lemma~9]{Lipy14}, the topological space $W \times [0,1]$ can be given the structure of a manifold with corners, which we will write $\Cre(W)$, satisfying $\bd \Cre(W) = W \sqcup -W^+ \sqcup -W^- \sqcup - \Cre(\bd W)$.
...
\greg{This was an old footnote in the definition. Do we still need some version of this? Do we need creasing defined at arbitrary points?:
Given instead a smooth $\phi \colon W \to \R$ with regular value at $0$ as in the preceding examples, we may alter $\phi$ to have image in $(-1,1)$ by composing with a diffeomorphism, for example $x \mapsto \frac{2\arctan(x)}{\pi}$.
	Up to isomorphism, this does not change $W^0$, $W^+$, $W^-$, and, up to diffeomorphism, the creasing construction does not depend on the choice of diffeomorphism $\R \to (-1,1)$ so long as $(-\infty,0]$ maps to $(-1,0]$ and $[0,\infty)$ maps to $[0,1)$.
	Similarly, there is nothing special about the value $0$ or the interval $(-1,1)$; given any $\phi \colon W \to \R$ with regular value $p$, we could construct a version of $\Cre(W)$ that creases along $\phi^{-1}(p)$ rather than $\phi^{-1}(0)$. }
...
See \cref{F: creasing} for a sketch of a creasing homotopy of the teardrop manifold. \greg{We need to replace $f^{-1}(p)$ in the diagram with $\varphi^{-1}(0)$.}
