...
\begin{definition}\label{V: maps are co-oriented}
	...
	If $V$ and $W$ are manifolds over $M$ that are disjoint\footnote{See \cref{E: copies,D: prechain sum} for the case where $V$ and $W$ are not necessarily disjoint.} as submanifolds of $\R^\infty$, we let $V \sqcup W$ be a manifold over $M$ with reference map $r_{V \sqcup W}$ such that $r_{V \sqcup W}(x) = r_V(x)$ when $x \in V$ and $r_{V \sqcup W}(x) = r_W(x)$ when $x \in W$.
