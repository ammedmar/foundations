...
\begin{example}
	...
	To be technically accurate, the space $V \sqcup V$ is not clearly defined as a manifold with corners, as we have defined our manifolds with corners in \cref{D: MWC} to be submanifolds of some $\R^N$. We solve this problem below by working, usually implicitly, with isomorphism classes of maps. 
	In this case we could take $V \sqcup V$ to mean $V \sqcup W$ where $V$ and $W$ are isomorphic as manifolds over $M$, and such choices all lie in a unique isomorphism class. 
	We will usually do this without comment and abuse notation by continuing to write $V \sqcup V$. 
\end{example}
