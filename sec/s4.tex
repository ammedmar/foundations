...
\begin{proof}
	...
	By \cref{L: co/chains well defined}, $n W \in Q_*(M)$, so $n W$ is the union of a trivial prechain and a degenerate prechain, $nW = T \sqcup D$ with $T$ trivial and $D$ degenerate.

	Again as in the proof of \cref{L: trivial structure}, as $T$ is trivial, it can be decomposed into connected components that are trivial or that form a trivial pair with another connected component.
	First suppose the connected component $V$ of $T$ is trivial.
	Then $V$ is isomorphic to one of the $W_i$.
	But then $W_i$ is trivial and so are all the components of $n W_i$.
	On the other hand, suppose $V$ is not itself trivial but that it is part of a trivial pair with another component $V'$.
	If $V$ is isomorphic to $W_i$ and $V'$ is isomorphic to $W_j$, then $i \neq j$, since otherwise, denoting isomorphism by $\cong$, we have $W_i \cong V \cong -V' \cong -W_i$, but we have assumed $V$ is not trivial.
	So instead we must have $W_i \cong - W_j$, and these form a trivial pair.


	\begin{comment}Representing $\uW$ by $W$, we can decompose $W$ as in the proofs of \cref{L: trivial structure,L: Lipy12} so that $W = W_1 \sqcup W_2 \sqcup \cdots$ with each $W_i$ being the union of isomorphic-up-to-sign connected manifolds over $M$ with the isomorphism classes of components in $W_i$ and $W_j$ distinct f $i\neq j$.
	Then $n \uW$ is represented by taking $n$ copies of each $W_i$ if $n > 0$ or, if $n < 0$, taking $|n|$ copies of each $-W_i$.
