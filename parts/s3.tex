...
\begin{proof}[Proof of \cref{L: pullback co well defined}]
	...
	Next, by the same argument by which embeddings $e$ exist, there is an embedding $\td H \colon V \times I \into M \times \R^N \times \R^Q$ for some $Q$ so that if $\td \pi \colon M \times \R^N \times \R^Q \to M \times \R^N$ is the projection then $\td \pi \td H = H$.
	...
	Additionally, $\pi \td \pi:F_j(x,t) = f(x)$
%	This seems like a typo. Please check
	so each homotopy is constant in $I$ when projected to $M$, and in particular each of $(e_0,0)$, $(e_1,0)$, $\td e_0$, and $\td e_1$ is an embedding $V \into M \times \R^{N+Q}$ over $f \colon V \to M$.
...
\begin{proof}[Proof of \cref{P: graded comm}]
	...
	So $(V \times I) \times_M W$ can be co-oriented, and by \cref{leibniz} two of its boundary components will be $f \times_M g \colon (V \times 0) \times_M W = V \times_M W \to M$ and
	$f' \times_M g \colon (V \times 1) \times_M W = V \times_M W \to M$, topologically, with the co-orientations given by composing the boundary co-orientations with the co-orientation of $F \times_M g$.
	Similarly, two of the boundary components of $W \times_M (V \times I)$ will be $g \times_M f \colon W \times_M (V \times 0) = W \times_M V \to M$ and
...
\begin{proposition}\label{P: co-oriented exterior unit}
	Let $f \colon V \to M$ be a co-oriented map of manifolds with corners, and let $g \colon pt \to pt$ be the unique map with the canonical co-orientation.
	Then $f \times g \colon V \times pt \to M \times pt$ and $g \times f \colon pt \times V \to pt \times M$ are each isomorphic as co-oriented maps of manifolds with corners to $f \colon V \to M$.
\end{proposition}
...
\begin{proposition}\label{P: cap cross}
	Let $f \colon V \to M$ and $g \colon X \to N$ be co-oriented maps from manifolds with corners to manifolds without boundary, and let $h \colon W \to M$ and $k \colon Y \to N$ be maps with $W$ and $Y$ compacted oriented manifolds with corners.
...
\begin{proof}
	...
	\begin{equation}\label{E: fiber plus euclidean}
		\Psi \colon P \oplus M \oplus \R^{2a} \to V \oplus W \oplus \R^{2a},
	\end{equation}
