...
In each case, the product is defined when the reference maps $r_V:V \to M$ and $r_W \colon W \to M$ are transverse.
\end{definition}
To define these products, in each case we suppose transverse maps $r_V:V \to M$ and $r_W \colon W \to M$ representing elements of
...
If $W \in PC_*^{\Gamma}(M)$ represented by $r_W \colon W \to M$, then $\Cre(W)$ is also compact and oriented, and we have a map $r_{\Cre(W)} = r_W\pi_W:W \to M$ with $\pi_W$ being the projection of $\Cre(W) = D\times_{(-1,1)}W$ to $W$. Thus $r_{\Cre(W)}:\Cre(W) \to M$ gives us a new element of $PC_*^\Gamma(M)$.
