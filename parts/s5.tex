...
\begin{lemma}\label{L: pullback map Q}
	Suppose $N$ is a manifold without boundary and $r_S:S \to N$ is trivial or has small rank.
...
\begin{proof}
	...
	Similarly, suppose $r_S:S \to N$ has small rank.
	Suppose at the point $x \in S$ we have $v \in \ker(D_x r_S)$.
	The tangent space of the pullback is the pullback of the tangent spaces \cite[Theorem 5.47]{Wed16}, so $T_{(x,y)}(S \times_N M)$ is the pullback $T_{(x,y)}P = T_xS\times_{T_{(r_S(x),g(y))}N} T_y M$, and we are interested in the map from this space under the derivative of the projection $\pi:S \times M \to M$.
...
\begin{theorem}\label{T: relative MV}
	...
	For any pair of open sets $U$ and $V$ in $M$ there are Mayer-Vietoris exact sequences \red{Fix tikz arrows?}
	\begin{equation*}
		\begin{tikzcd}[
			column sep = small,
...
\begin{proof}
	...
	Next, suppose $\uW$ is a cocyle in $S^*$, represented by $(W_1,W_2) \in PC^*_\Gamma(M|_U) \oplus PC^*_\Gamma(M|_V)$, that maps to zero in $H_{\Gamma}^*(M|_{U \cup V})$.
	Thus there is
...
\begin{proof}
	...
	\end{diagram}
	The horizontal maps are isomorphisms because these are homotopy functors, and the lefthand vertical map is an isomorphism because $\Phi$ takes a generator of $H_0(pt) \cong \Z$ to a generator of $H_0^\Gamma(pt) \cong \Z$; see again \cref{E: dimension}.
	So the righthand map is also an isomorphism.
...
\begin{proof}
	...
	We will define cube-wise a chain homotopy inverse $s \colon NK_*(M) \to NK^{sm}_*(M)$ by starting with a map $\td s:SK_*(M) \to SK^{sm}_*(M)$ and passing to quotients.
	...
	We can now define $\td s:SK_*(M) \to SK^{sm}_*(M)$ by $\td s(\sigma) = H_\sigma(-,1)$.
