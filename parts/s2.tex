...
\begin{remark}\label{R: multiproducts}
	...
	% anibal: this sentence sounds clunky, maybe:  we can form the \textit{transverse} iterated fiber product $(((V_1\times_MV_2)\times_M V_3)\times_M\ldots)\times_M V_n$ as well as in any other order of association.
	% GBF: Done.
...
Let $f \colon V \to M$, $g \colon W \to M$, and $h:Z\to M$ be maps from manifolds with corners to a manifold without boundary. Suppose that $W$ is transverse to $Z$ and that $V$ is transverse to $W$ and to $W\times_MZ$. Then $V\times_MW$ is transverse to $Z$. In particular, if $V\times_M(W\times_MZ)$ and $V\times_MW$ are well defined, then so is $(V\times_MW)\times_M Z$.
\end{proposition}
\begin{proof}
